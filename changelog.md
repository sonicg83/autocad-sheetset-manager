...
- 根据评审重写 `PLAN-INT-004`：不再设计契约测试、分层拆除或数据库迁移，改为把 Builder 源码、前端、测试、独立迁移、插件、打包入口和全部专用脚本整体移入本地 `legacy/dst-builder/`。
- 计划仅保留“整体归档并清理入口”和“历史文档封口”两个任务；明确保留 Manager 的 0007/0008 历史迁移与 `dst_platform`，并同步 RFC 和执行资料索引。
