...
- 验证：`npm --prefix web run test:e2e -- tests/e2e/extensions-navigation.spec.ts --workers=1` **6 passed**；`npm --prefix web run test:unit` 35 passed；`check:api`/`check:i18n`（784 键 / 9 域）/`vue-tsc`/`vite build` 通过；`main.spec.ts` 回归通过（见提交前运行记录）。
