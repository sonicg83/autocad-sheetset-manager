...
- 实施 [PLAN-DM-043](.planning/plans/dst-manager/PLAN-DM-043-table-alignment-frontend-remediation.md) Task 1（建立表格单元格静态门禁）：新增 `web/scripts/ui-contracts/table-cells.mjs`，为 `check:ui` 加入 `table-cell-vertical-align`（所有 `th/td` 规则必须显式声明，取值仅 middle/top）与 `table-cell-padding`（单档、令牌化、禁裸值与普通格零 padding）两条规则，跨列结构零 padding 由模块内 `STRUCTURAL_CELL_PAIRS` 精确配对放行（不进例外表）；新增 13 条契约用例（RED `pass 4 / fail 9` → GREEN `pass 13`）与 5 条 CLI 级变异证据（分类 15 → 20、注入 17 → 22），`test:contracts` **114 passed / 0 failed**；逐条量取 11 个含表格组件 + legacy 兜底的单元格现状（只有图纸/属性主表命中 44px 档，其余 16 张表均无 height 声明）并登记 27 条存量例外（总数 10 → 37，到期任务 Task 2/3/4/5 分别为 16/2/8/1），`check:ui` 退出 0；真实仓库变异探针验证新表格立即失败后已删除。
- 实施 [PLAN-DM-043](.planning/plans/dst-manager/PLAN-DM-043-table-alignment-frontend-remediation.md) Task 2（图纸与属性主表单元格几何）：两表普通单元格 padding 由裸值 `10px 8px` 收敛为单档令牌 `var(--space-2)`，并按 ARCH-DM-007 §4.3 为两表 14 条辅助 `th/td` 规则补显式 `vertical-align:middle`（`td.col-default` 保持 middle，零视觉变化）；结构行 `.sheet-editor-row>td` 继续零 padding 并由内层 `.sheet-property-editor` 消费 `--space-4`。两份 e2e 先是 `padding-top` 10px ≠ 8px 的 RED，改后 `sheets-layout` + `properties-visual-evidence` **81 passed / 0 failed**；`check:ui` 先报 16 条 stale-exception 提醒清退旧例外，清退后 exit 0（例外总数 37 → 21）。
- 实施 [PLAN-DM-043](.planning/plans/dst-manager/PLAN-DM-043-table-alignment-frontend-remediation.md) Task 3（常驻编辑表 48px 档与出错行增高）：`tokens.css` 新增组件令牌 `--editable-table-row-height:48px`，标准普通属性编辑表与创建向导分组表的单元格声明 `height` + `padding:var(--space-1)` + `vertical-align:middle`；出错行加行级类 `tr.has-issue` 整行切 `vertical-align:top`，并以令牌派生的 `margin-top:calc((var(--input-height) - …) / 2)` 为行内小于 38px 的控件（32px 命中区、36px 图标按钮、行操作按钮）补半高差，使出错行内控件中点差 ≤1px 且不拉伸控件本体。RED 实测表头 26px/41px ≠ 48px、出错行中点差 3px；GREEN 后 `standards-editor` + `create-sheetset-input` **81 passed / 0 failed**、`npm run build` 退出 0，清退 Task 3 的 2 条例外后 `check:ui` 退出 0。
- 实施 [PLAN-DM-043](.planning/plans/dst-manager/PLAN-DM-043-table-alignment-frontend-remediation.md) Task 4（其余标准/创建/目录只读表）：标准资产布局表、创建复核预览表、图纸值详情表与图纸目录预览表统一声明 `height:var(--sheet-table-row-height)`（44px 档）+ `padding:var(--space-1)` 单档令牌 + 显式 `vertical-align:middle`；复核预览表的张数列改为右对齐并启用 `tabular-nums`（表头同步，图号/范围仍左对齐）；补 `asset-layout-table`／`sheet-values-table`／`catalog-preview-table` 三个稳定锚点；布局表空态说明行按普通格同档、不因 `colspan` 放行。RED 实测表头 26px/40px ≠ 44px 且张数列未右对齐；GREEN 后三份 e2e **132 passed / 0 failed**、`npm run build` 退出 0，清退 Task 4 的 8 条例外后 `check:ui` 退出 0（例外 19 → 11，仅剩 Task 5 的 1 条）。
- 实施 [PLAN-DM-043](.planning/plans/dst-manager/PLAN-DM-043-table-alignment-frontend-remediation.md) Task 5（旧页面表与 legacy 收口）：`PreviewPanel`（8 张表）、`JobStatusPanel`、`RevisionHistoryPanel` 三个组件新建 scoped 样式，统一 44px 档 + `padding:var(--space-2)` 单档令牌 + 显式 `vertical-align`；任务进度/耗时为可比较数值列（右对齐 + `tabular-nums`）且单位入表头（中英文 `进度 (%)` / `耗时 (ms)`），单元格只输出数值并删除失去引用的 `jobs.files.durationMs`；受影响张数列右对齐 + `tabular-nums`；跨列日志详情行取 `top` 并允许换行；**删除 `legacy.css` 的全局 `th,td` 兜底规则**（11 个含表格组件共 18 张表已全部自持几何）。验证：`main.spec.ts` **132 passed**、全量 `test:e2e` **697 passed / 0 failed**、`test:contracts` 114/114、`test:unit` 340/340、`npm run build` 与 `ruff check` 退出 0，PLAN-DM-043 的例外到期项全部清退（`check:ui` exit 0）。真实 Windows WebView2 100/125/150/200% 复验未执行，计划保持 `active`。
- PLAN-DM-043 收尾：8 个提交以 fast-forward 合入 `main`（`main` == `b6ee65f`，历史保持线性），合入后复跑全量门禁 **697 passed / 0 failed**、`test:contracts` 114/114、`test:unit` 340/340、`build` 与 `ruff check` 退出 0；因真实 Windows WebView2 100/125/150/200% 缩放复验未执行，计划保持 `active`，特性分支按「不擅自删除分支」保留。
- 整支复核与规范同步：PLAN-DM-043 收口后 [ARCH-DM-007](docs/dst-manager/architecture/ARCH-DM-007-frontend-ui-foundations.md) §4.3 与 [SPEC-DM-006](docs/dst-manager/specs/SPEC-DM-006-dst-manager-desktop-ui-ux.md) §6.4 中"存量 `10px 8px`／`9px` 档按 §7 收口"的表述已与本轮实现及新静态门禁矛盾（门禁会拒绝这两档裸值），两处补注"PLAN-DM-043 已收口：现役只保留 `--space-2`（8px）与 `--space-1`（4px）两档，legacy `9px` 与 `:where(#app) th,td` 兜底规则已删除"，SPEC §1.1 追加对应修订记录并订正计划登记的 RED 计数；整支复核记录（0 Critical / 1 Important / 5 Minor 与各项裁决）归档为 [MEMO-DM-042](.planning/memos/dst-manager/2026-09-26-plan-dm-043-review.md)。验证：`check:ui` 退出 0、`check:i18n` 1622 键对称、`test:contracts` 114/114。
...
- 同一预检凭证的确认、取消与清理串行，重复并发确认返回同一成功结果；导入弹窗在关闭、换文件和重新预检后丢弃旧响应并取消旧凭证，避免路径与待确认包错位。
- 新增后端损坏包、冲突快照、到期清理、同凭证并发确认，以及前端旧响应与重新预检的回归测试。
- 验证：`uv run ruff check .`、相关 pytest（225 passed）、前端单测（29 passed）、生产构建及 Playwright（59 passed）通过。`uv run pytest -q` 仅有 `tests/unit/test_setup_bat.py` 两条未改动用例失败：当前环境运行 GBK 批处理时输出含替换字符，中文输出断言不成立；排除这两条后全量测试通过。

## 2026-09-25（按项目现状修订根 README）

- 根目录 `README.md` 与 `README.en.md` 同步项目现状：核心特性新增图纸标准平台（官方/用户标准库、普通/派生/组合属性分区编辑、DWG 命名模板、模板资产与发布检查、服务端分配正整数版本与名称唯一门禁）、标准包 `.dststandard` 导入导出（原生选择器 + 限时快照预检/凭证确认两步导入）与标准驱动新建图纸集（四阶段创建向导、可恢复创建草稿、XLSX 结构导入、独立创建发布事务与工作区接管）；引言补充标准库定位。常用验证命令注明 pytest 默认经 pytest-xdist 并行；文档导航补充 GUIDE-DM-007 官方标准包发布指南。仅文档修订，未改动代码。
...
- 修复 F14：列表加载失败、详情加载失败各自就地重试（重发对应请求，禁用重复提交），筛选无结果提供「清除筛选」并恢复默认筛选（不改变当前选择）；新文案中英对称。
- 验收 E2E 追加：900×768 两级互斥与返回保留筛选/选择、200% 缩放（720×450@2x）下返回按钮可达且无页面级横向溢出、键盘 Enter 进入与返回、超长中英文标准名不溢出、列表与详情失败就地重试。
- 验证：RED 2 例（列表在详情打开时仍可见、无重试/清除入口）；GREEN `npm --prefix web run test:unit` 314 例、标准库 spec 51 例、其余标准 spec 79 例、`npm --prefix web run build`（check:api/check:i18n/check:ui/vue-tsc）均通过。
...
- 修复 1（Important，创建任务全程不续租）：`CreationJobRunner` 从认领到终态只在 4 个状态迁移点写 `jobs` 行，`_stage_groups` 逐组跑真实 Core Console 期间无任何心跳，因此另一个 service 实例（手动 `dst-manager worker`、`doctor`、`open`）调用 `recover_stale_jobs` 时会按 `REQUEUED_SAFE_STAGE` 回收在飞任务、清空 `worker_id`：发布前后所有围栏写入随即被拒，任务停在 `QUEUED`、目标却已建好，下一次认领因目标非空误报 `CREATION_TARGET_NOT_EMPTY`。修复：新增 `_LeaseRenewal`（与 `CadJobRunner` 同一 `heartbeat_interval` 口径 `min(30.0, lease_seconds / 3)`、同一失败语义），在「每组完成后」与「发布前」两个边界续租（发布前 `force=True` 实测所有权，发布是不可逆动作不适用节流），失败抛同码 `CREATION_JOB_LEASE_LOST` 并立即停手；`stage()`（无任务库的隔离单测入口）不续租，`run` 走新的 `_stage_guarded`。生产装配 `creation_execution.run_creation_job` 与 `service.run_next_job` 的 `CadJobRunner` 用同一表达式取值。已知上界：单次 CAD 调用超过租约时仍会失去租约，但新实现会在下一边界立即停手（不发布、不落终态），重试干净重跑，不再出现「成果已发布而任务停在 QUEUED」。回归测试 1 例（`tests/integration/test_creation_job.py::test_run_renews_the_lease_while_staging`）：替身执行器把任务行 `heartbeat_at` 改到 10 分钟前（模拟「另一实例眼中已超期」的在飞任务），并在第二组 CAD 后由第二实例调用 `recover_stale_jobs(30)`；`heartbeat_interval=1e-6` 使两个续租边界必定到期，用例不依赖真实等待时长与调度。**RED 证据（去掉续租后实跑，等价修复前实现）**：`assert [{'conclusion': 'REQUEUED_SAFE_STAGE', 'id': 'job-1'}] == []`——在飞任务被并发恢复回收。**变异验证（实跑）**：只去掉发布前 `lease.refresh(force=True)` → `assert 2 == (2 + 1)`（发布前续租是独立且必要的边界）。首版用例用「1s 租约 + 睡眠 1.05s」，在 xdist 并行下因调度延迟多出一次到期续租（实测 `3 != 2`）而抖动，故改为上述确定性设计。
- 修复 2（本计划新增的「用户文本进 SCR」seam 零直接测试）：`tests/unit/test_autocad_worker.py` 追加 **9 例**覆盖 `render_create_layouts`/`LayoutCreationRequest`——固定脚本结构（`_.NETLOAD`/`DstGetLayoutHandles`/`_.QSAVE`/`_.QUIT` 各恰好一次、`DstGetLayoutHandles` 晚于 `DstDeleteDefaultLayout`、临时名与目标名两段改名），含空格与中文的布局名/图幅/模板路径一律以引号包裹的参数出现，以及含 `"`/换行/NUL/空图幅/空目标列表/重复（含大小写不同的 `A1`/`a1`）目标布局名在**构造期**即以 `LAYOUT_CREATION_REQUEST_INVALID` 拒绝。变异验证（故意破坏实现后实跑）：去掉构造期 `encode_scr_argument` 门禁 → 4 个非法输入用例失败；目标布局名不加引号 → 结构用例失败；`DstGetLayoutHandles` 提前到删除默认布局之前 → 结构用例失败；去掉重复名门禁 → 2 个重复用例失败。
- 修复 3（Minor 升级，候选列表对损坏标准文档冒泡 500）：`creation_assets._candidate` 只捕 `StandardSchemaError`，截断/非 UTF-8 的已发布 `document.json` 会让 `GET /api/creation-drafts/standards` 500。修复：`except (StandardSchemaError, OSError, ValueError)`（`ValueError` 覆盖 `json.loads` 的 `JSONDecodeError` 与 `UnicodeDecodeError`）→ 返回 `available=False` + 稳定原因；非 UTF-8 文档更早地在 `StandardStore._read_document`（`GET` 列表路径，非 `get`）触发 `read_text` 的 `UnicodeDecodeError`，同一条「已发布文件是不可信输入」口径下把该处 except 由 `(OSError, json.JSONDecodeError)` 收为 `(OSError, ValueError)`（原本该处会 500 的同一缺陷类）。回归测试 2 例（截断 + 非 UTF-8）断言列表仍 200、该标准 `available=False` 且带稳定原因。
- 修复 4（裁决，目录名为空不得退化为上级目录）：`creationTargetPath` 原在 `folder === ""` 时返回上级目录，用户清空「项目目录名」后 `target_path` 变成上级目录本身，若该目录恰为空项目就会落在上一级目录里（`AcSmSheetSet.Name` 取该目录名）。修复：目录名为空返回空串，由既有 `CREATION_TARGET_PATH_EMPTY` 诊断与 `finalPathEmpty` 提示承担（不新增错误码）；`finalPathEmpty` 中英文案同步改为同时覆盖「未选上一级目录」与「目录名为空」，不再断言「尚未选择上一级目录」。前端单测 1 例：清空目录名后 `targetPath()` 为空、保存以空路径落盘、后端 `executable=false` 时 `canExecute=false` 且 `execute()` 不发请求。**RED 证据（修复前实跑）**：`AssertionError: expected 'D:\项目' to be ''`（即旧行为退化为上级目录）。
- 验证（逐行执行）：`uv run ruff check .` 通过；`uv run pytest` **1900 passed / 74 skipped / 0 failed**（基线 1888 / 74，+12 例：修复 1 的 1 例、修复 2 的 9 例、修复 3 的 2 例）；`uv lock --check` 通过（Resolved 73 packages，无变更）；web/ 下 `npm run test:unit` **303 passed / 28 files**（基线 302，+1 例）、`npm run check:i18n` **1571 键 / 11 域**对称、`npm run check:ui` 通过、`npm run build` 通过、`npx playwright test create-sheetset-input.spec.ts create-sheetset-review.spec.ts` **58 passed**。未改 `CadJobRunner`（只读参考）与既有普通任务行为。

## 2026-09-24（补齐编号设置失效接线与诊断定位，PLAN-DM-036 Task 9 审查修复）

- 修复审查发现（Important 1，plan-mandated）：brief Step 4「编号设置变化立即禁用旧预览」在生产路径上没有实现——`invalidateForSettingsChange` 全仓只有定义、暴露与单测调用，没有生产调用者（死代码）；设置中心入口常驻且未加载工作区同样可用，向导停在第四阶段时可改 `unnumbered_subset_keywords`／标题尾序号开关与类型，此时页面仍展示改前的「当前编号设置」摘要且「创建并打开图纸集」仍可点，只有点击后才由后端以 409 `CREATION_PREVIEW_STALE` 拒绝。最小修复：`CreateSheetSetView.vue` 对生效编号设置（`useSettings` 的模块级单例快照里的 `enable_add_number_suffix`／`number_suffix_type`／`unnumbered_subset_keywords` 三个键）做一次 `watch` 并调用 `store.invalidateForSettingsChange()`；快照首次加载（null → 值）只是读取、不是设置变化，不失效（否则打开设置中心就会清掉仍然有效的预览）。**未新建全局状态、未轮询**，复用既有设置读取；未放宽后端 `CREATION_PREVIEW_STALE` 门禁，也未改动既有普通任务行为。
- 修复审查发现（Important 2，plan-mandated）：组内「模板/图幅」诊断的定位落在图名控件上——后端对张数/基础模板/布局模板/图幅的组内诊断都不带 `property_id`，前端 `previewDiagnosticTarget` 于是只给出「图纸组阶段该行」，`focusGroupTarget` 在无具体属性时取行内首个控件，而 DOM 顺序是 复选框 → 图名 → 张数 → 基础模板 → 布局模板 → 图幅，故焦点落在图名而不是诊断所指的模板/图幅（brief Step 3 与 SPEC-DM-018 §7 要求可定位到模板/图幅）。最小修复：`CreationPreviewTarget` 增加字段判别 `groupField`（`title`/`count`/`base`/`layout`/`paper`），按诊断码映射到对应控件；基础模板与布局模板资产共用 `CREATION_ASSET_INVALID`，故用该组已解析的模板路径（空串表示该资产解析失败）区分到底缺的是基础模板还是布局模板（两者都缺时先指向基础模板）。页面层 `focusGroupTarget` 按既有可访问名拼写（`第 N 组<列名或属性名>`）定位，不新建第二套 DOM 约定；未改后端诊断码与契约。
- 测试（TDD，先 RED 后 GREEN，均实跑）：`web/tests/e2e/create-sheetset-review.spec.ts` 追加 **2 例**——① 生效编号设置变化后旧预览失效：仅打开设置中心（快照首次加载）仍可执行，改不编号关键字并保存后 `creation-preview-stale` 出现、编号摘要与执行入口消失、预览请求数仍为 1（不自动重算）；② 模板/图幅类组内诊断跳回后焦点落在对应控件：基础模板资产非法 → 该行基础模板选择（并显式断言不是图名）、图幅不在布局模板内 → 该行图幅选择、布局模板资产非法 → 该行布局模板选择。`web/tests/e2e/fixtures/creation.ts` 新增 `creationPreviewWithTemplateDiagnostics()`（三种模板/图幅诊断 + 对应的已解析模板路径），`fixtures/settings.ts` 新增 `numberingSettingsSnapshot()`/`installNumberingSettings()`（含三个编号键的最小快照与 PUT 回写）。`web/src/features/creation/previewModel.test.ts` 为 `previewDiagnosticTarget` 补 2 例（张数/图幅码映射；`CREATION_ASSET_INVALID` 按已解析模板拆分基础/布局模板）并按新增字段更新既有断言。RED 证据：两例 e2e 分别报 `creation-preview-stale` 元素不存在（未失效）与「第 1 组基础模板」`inactive`（焦点落在图名）；实现后均转绿。
...
## 2026-09-24（实现按组预览、逐张属性模态与创建闭环，PLAN-DM-036 Task 9）

- 第四阶段「检查并创建」接入后端权威预览（`web/src/components/creation/ReviewStep.vue`，新增）：顶部显示固定标准及版本、最终项目路径、图纸组数/总张数/DWG 数、当前编号设置摘要（序号字段/位数/起始、标题尾序号、不编号关键字）与检查状态；主表**每个图纸组一行**，固定列为 `图纸组｜图纸范围｜图纸｜张数｜文件名｜基础模板｜布局模板｜图幅｜标准动态 sheet 属性列`，**不提供「布局」列与单张 Sheet 行**。图纸范围、紧凑标题（如 `平面图 (一)-(三)`）、DWG 文件名（带 `.dwg`）与逐张属性值全部原样取预览响应，前端不推算序号、不拼凑文件名；不编号组只显示同位数单值（如 `00`，不显示 `00-00`）。动态属性列的身份与顺序取后端 `property_cells` 键、名称取当前标准的属性定义（标准未声明的属性以属性 ID 兜底显示，不静默丢列）；组内取值相同直接显示值，不同时显示第一张实际值 + 可点击「…」，首张为空显示「（空）…」，**绝不显示「多值」**。
- 逐张属性模态（`web/src/components/creation/SheetValuesDialog.vue`，新增）：标题为「图纸组 · 属性名」，按组内顺序列出**全部图纸**的「图号｜图纸标题｜该属性实际值」（空值显示「（空）」），只展示所点属性、不改变主表一组一行的结构；复用 `useDialogFocus`（打开送焦点、Tab 圈定、Esc 与关闭按钮退出、关闭后焦点归还触发按钮）。
- 预览纯模型与预览会话（`web/src/features/creation/previewModel.ts` + `previewSession.ts`，新增）：前者是无状态投影（`summarizeSheetValues`、`previewPropertyColumns`、`previewDiagnosticTarget`、`splitPreviewDiagnostics`），后者是「按需保存 → 请求权威预览 → 写入摘要与可执行标志 → 按摘要执行」的状态迁移。`store.ts` 只组合两者（478 → 509 行），对外新增 `preview()`/`execute()`/`invalidateForSettingsChange()`，`CreationApi` 新增 `previewDraft`/`executeDraft`（`api/creation.ts` 手写防御性映射，`schema.d.ts` 无漂移）。任何草稿输入、标准身份或编号设置变化都清空 `previewState`/`previewDigest`/`canExecute`；执行**只发送 `preview_digest`**，无有效预览时不发请求。
- 诊断与执行门禁：诊断按后端 `severity` 分区为阻断错误与非阻断提示，可定位的错误给出「返回修改第 N 项」——带图纸组 → 图纸组阶段该行（带属性时定位到该属性控件），带图纸集属性 → 项目信息阶段该字段，路径诊断 → 路径字段，空图纸组 → 图纸组阶段，标准资产文件不可用 → 选择标准阶段，其余（如布局名重复）只呈现消息不给误导性跳转。仅当后端预览无阻断错误时可创建，创建前经共享确认模态展示最终路径并要求勾选不可覆盖确认（`requireCheckbox` + `irreversible`）。
- 任务闭环（`web/src/features/creation/useCreationJob.ts`，新增）：创建任务在登记前 `workspace_id` 为 NULL，而 `useJobMonitor` 按工作区身份匹配，因此补一层创建专用订阅（SSE 优先、断线回退轮询），**复用 `useJobMonitor` 导出的终态集合**与 `JobStatusPanel.vue` 组件本体。失败保留草稿与可读诊断并使旧摘要失效（重新检查后以新任务重试）；成功由向导把 `workspace_id` 交给 App，经新增的生命周期入口 `useWorkspaceLifecycle.openWorkspaceById` 接管普通工作区（无工作区时按 ID 拉取、已有其它工作区时不静默替换）。向导卸载即清除本地草稿身份，不再提示恢复已交付的草稿。
- 修复：`interfaces/api.py` 的 SSE 终止集合漏掉创建任务可终局的 `NEEDS_REVIEW`（任务已终局而事件流继续轮询，客户端只能靠断线回退收到终态），改为复用 `infrastructure.persistence.database.TERMINAL_JOB_STATUSES`；新增 `tests/unit/test_job_terminal_statuses.py` 钉住「后端 SSE 用共享常量」与「前端 `useJobMonitor.TERMINAL_JOB_STATUSES` 与后端逐项相同」。另修复草稿保存的并发竞态（`store.ts` 保存串行化）：进入第四阶段时「阶段切换保存」与「预览前的按需保存」几乎同时发出、拿同一个 `expected_revision` 撞后端乐观修订门禁（实测 409）。
- 测试（TDD）：新增 `web/src/features/creation/previewModel.test.ts`（21 例，含 brief Step 1 两个逐字用例——`summarizeSheetValues` 首张值 + 多值标记、编号设置变化后 `previewDigest`/`canExecute` 失效）与 `web/tests/e2e/create-sheetset-review.spec.ts`（8 例：权威摘要与按组一行主表、同值/多值/首张空值单元格、逐张属性模态（键盘打开/Esc/关闭按钮/焦点回归）、输入变化后不复用旧摘要、阻断错误与非阻断提示分区与定位跳转、只发 digest 的执行与不可覆盖确认与成功后工作区切换、失败保留草稿可重试、900×768/200% 缩放/浅深主题无整页横溢）。`web/tests/e2e/fixtures/creation.ts` 追加预览/执行/任务 SSE 与轮询端点 mock（预览响应是后端权威结果的替身）；`create-sheetset-input.spec.ts` 的第四阶段占位断言改为断言真实预览主表。TDD 证据：先写用例实跑 RED——`npm run test:unit -- src/features/creation/previewModel.test.ts` 报 `Cannot find module './previewModel'`、`npx playwright test create-sheetset-review.spec.ts` **8 failed / 33 passed**（断言占位说明与缺失的预览表）；实现后逐轮转绿。
- 验证（逐行执行）：`uv run ruff check .` 通过；`uv run pytest -rs -p no:warnings` **1888 passed / 74 skipped / 0 failed**（272.15s；基线 1885，+3 例为本次新增的后端测试）；`uv lock --check` 通过；web/ 下 `npm run check:api` 通过（`schema.d.ts` 无漂移）、`npm run check:i18n` **1571 键 / 11 域**对称、`npm run check:ui` 通过（0 违规、无 stale 例外）、`npm run test:unit` **300 passed / 28 files**（基线 279，+21 例）、`npm run build` 通过（vue-tsc + vite）、`npm run test:e2e` **654 passed / 0 failed / 0 flaky**（最终提交状态全量；此前两次全量分别为 651 passed / 2 failed / 1 flaky 与 652 passed / 2 flaky，失败与 flaky 项全部是 `page.goto` 被中断的既有 vite dev server 并行抖动——两个 spec 单独重跑 141 passed）；`powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build_plugins.ps1` **0 警告 / 0 错误**（2016 与 2020 双版本重建）。真实 AutoCAD 2016/2020 系统测试由 Task 7 跑通（`tests/system_autocad/test_creation_workflow.py` **2 passed**，本机未重跑）；**官方图纸集管理器（SSM）界面打开性仍为人工残余验收项**，本任务未执行、不记为已通过。
...
- 前端不复制后端规则：属性求值、编号、DWG 命名、目标目录状态一律不重算，只保留 SPEC-DM-018 §3.1/§4.1 明确要求的前端即时提示（图名非空、去首尾空格 + 大小写不敏感的图名唯一、张数正整数、模板与图幅来自当前标准候选）；重复图名对同名的每个组都标错，不自动改名、不追加序号。标准默认值只在采用草稿时按「缺键才补」合并，用户主动清空写入的显式空串原样保留，恢复与保存都不回填。
- 入口与四阶段壳：`App.vue` 用 `CreateSheetSetView.vue`（265 行）替换原「该入口将在后续版本提供」占位并删除占位样式与对应语言键；`useStartNavigation` 增加一次性创建身份（`createSheetsetIdentity`，回欢迎页即清除），`StandardDetailPane` 的「用于创建」改为携带 `{standardId, version}`，`StandardsView` 只转发身份——标准详情入口因此固定发布版本并直接落到第二阶段。第一阶段只列已发布且依赖、模板资产可用的候选，一个都没有时集中列出不可用原因并提供「前往标准库」入口；标准草稿不进入候选。切换标准先经共享确认模态，确认后删除旧草稿、清除旧标准的字段/模板选择/预览，不静默迁移。
- 输入界面：`ProjectStep.vue` 按标准动态生成普通属性（文本/枚举、必填与标准默认值可见、枚举可显式置空），派生属性只读展示「待计算」；项目路径复用既有 `shell.select_folder` 桥选上一级目录 + 可编辑目录名（初值「新建项目」），展示拼接后的完整最终路径，桥不可用时按钮禁用并给出可见原因、保留手动输入。`GroupsStep.vue` 一行一个子集（图名｜张数｜基础模板｜布局模板｜图幅｜标准定义的其他可输入 sheet 属性｜操作），末列沿用图纸目录插件的 32×32 图标按钮视觉（↑/↓/✕ + 完整 aria-label，首末行禁用正确，已按 ARCH-DM-007 §6 登记三条 `unicode-structure-icon` 例外并注明到期条件）；新建组复制创建序（`created_order`）最大的组并聚焦图名，重排只改数组顺序不改变创建序；`GroupBatchDialog.vue` 只作用选中组，当前值不一致显示「值不相同」，清空必须走「明确清空」（空输入不当作清空），字段列表不含图名与派生字段。
- XLSX 辅助输入（`XlsxImportDialog.vue` + `web/src/api/creation.ts`）：模板地址取自草稿固定的标准版本；草稿已有用户输入时先经共享确认模态说明「全量覆盖项目属性、项目路径和全部图纸组」，取消不发任何写请求；导入失败整批零变更并按工作表/行/列定位诊断（文案优先取 Task 6 的 `creation.diagnostic.*` 码表，未知码回退后端文本）；成功后一次性替换输入、保留固定标准身份并使预览失效。前端不解析公式、不执行宏或外部链接，也不把 Data Validation 当作合法性证明。
- 草稿恢复：草稿身份记在本地（`dst-manager.creation-draft-id`），重新进入向导时恢复输入与当前阶段并提示「继续草稿」/「重新开始」（后者经确认后删除草稿并回到第一阶段）；返回欢迎页前先落盘，未保存输入不因离开而静默丢失。恢复失败（草稿已不存在或固定标准不可用）清除本地身份并从第一阶段重新开始。
- 测试：新增 `web/src/features/creation/store.test.ts`（4 例，含 brief Step 1 两个逐字用例）、`web/tests/e2e/create-sheetset-input.spec.ts`（13 例：四阶段壳与两入口、无可用标准与候选加载失败、动态属性与路径合成/无壳降级、复制最近创建组与重排、批量修改（混合值 + 明确清空 + 空输入不当作清空）、草稿恢复与重新开始、切换标准、XLSX 取消/失败/成功、键盘输入、浅深主题与 900×768 无整页横溢）、`web/tests/e2e/fixtures/creation.ts`（自洽 route mock：草稿增删改查带乐观修订门禁、模板、导入成功/整批拒绝、标准库列表与标准文档详情）。`standards-welcome.spec.ts` 两处占位断言改为断言真实向导入口（原「从标准创建图纸集入口当前明确不可用」用例的覆盖已由新 spec 的标准详情入口用例承接并删除）。
- TDD 证据：先写用例并实跑 RED——`npm run test:unit -- src/features/creation/store.test.ts` 报 `Cannot find module './store'`、`npm run test:e2e -- create-sheetset-input.spec.ts` 报 `No tests found`；实现后 GREEN。e2e 首轮 4 例失败均为真实缺陷：本地草稿身份被构造期 `immediate` watcher 清空（恢复流程随后找不到草稿）、详情面板空态与列表空态共用同一文案导致严格模式多元素命中、第二阶段无法退回第一阶段（`canGoBack` 误设为 `stepIndex > 1`）；三处修复后全绿。
- 验证（web/ 下逐行执行）：`npm run test:unit` **279 passed / 27 files**；`npm run check:i18n` **1522 键 / 11 域**对称；`npm run check:ui` 通过（0 违规；新登记 3 条图标例外后无 stale）；`npm run test:contracts` **96 passed**；`npm run build`（check:api + check:i18n + check:ui + vue-tsc + vite）通过；`npm run test:e2e` **644 passed / 0 failed**（含依赖工程 settings-dialog 33 例）。

## 2026-09-24（阻止把创建初始修订当作可恢复点，PLAN-DM-036 Task 7 审查修复）

- 修复审查发现（Important，plan-mandated）：初始修订登记进共享修订表后，既有「恢复为新修订」流程把它解释成「删除全部已发布成果」。证据链（实测复现）：创建修订的 `revision_dir` 是发布事务归档目录，其 `manifest.json` 是原样写出的发布日志（`publish_journal.archive_journal`），创建发布条目**没有** `backup`/`before_hash` 字段（`project_publisher._candidate_entries`），而普通发布条目有（`publisher`）；`preview_revision_restore` 仅凭 `entry.get("backup")` 判定 `action`（无 backup 即 `delete`、`restore_hash=None`），且不校验 `revision["kind"]`，刚创建项目哈希一致故 `conflicts` 为空、`executable=True`；`restore_revision` 对 `staged=None` 走删除分支（`publisher._commit_delete`），随后 `_committed_result_hash` 对删除条目抛 `COMMITTED_RESULT_MISSING` → 任务落 `NEEDS_REVIEW`，而项目 DST 与主 DWG 已被移出项目目录。可达面：`/api/revisions?workspace_id=` 不过滤 kind，界面对每条修订渲染恢复按钮。实测 RED（修复前）：预览 `executable=True`/`conflicts=[]`/三条 `action=delete`（DST + 2 个主 DWG，`restore_hash=None`）；直接调用执行入口后项目根目录只剩 `.dst-manager/`，任务 `NEEDS_REVIEW` + `COMMITTED_FINALIZE_FAILED`。
- 最小修复：`RevisionRestoreOperations` 新增共享门禁 `_require_restorable_revision`（工作区 404 / 修订 404 的既有语义不变；`kind` 不在 `_RESTORABLE_REVISION_KINDS`（`operation`）即 409 `REVISION_NOT_RESTORABLE`），**预览与执行两条路径**都在任何任务创建与删除分支之前调用它——只挡预览不足以防护可被直接调用的执行入口。新稳定码同批登记到 `message_catalog.py` 与中英文 `web/src/i18n/locales/*/errors.ts`（`errors.revision.notRestorable`），`tests/unit/test_message_catalog.py` 清单同步锁定。未改普通（`operation`）修订的恢复语义、错误码与诊断；未改发布器；未放宽「登记失败不得报成功」；函数名、既有错误码与日志格式不变。
- 测试：`tests/integration/test_created_project_opens.py` 追加 **1 例**（`test_initial_creation_revision_is_not_a_restore_point`）：对创建项目的初始修订，预览与执行两条路径都以稳定码 `REVISION_NOT_RESTORABLE` + 409 拒绝，接口层 `GET /api/workspaces/{id}/revisions/{id}/restore-preview` 同样 409 + 稳定码，且项目 DST 与全部主 DWG 的文件哈希零改动、修订行不增长、无 `revision_restore` 任务、无残留工作区写锁。先 RED（`DID NOT RAISE`）后 GREEN。
- 顺带修正本任务条目的可核验数字：新测试文件 `tests/integration/test_created_project_opens.py` 实为 **641 行 / 13 例**（原记「607 行，12 例」），全套基线实为 **1884 passed / 74 skipped / 0 failed**（原记「1957 passed」把收集总数 1958 当成了通过数；本机在 `67b6446` 上实测复核）。
- 验证：`uv run ruff check .` 通过；`uv run pytest tests/integration/test_created_project_opens.py tests/integration/test_api.py tests/unit/test_core.py tests/unit/test_message_catalog.py` **260 passed / 0 failed**；`uv run pytest` **1885 passed / 74 skipped / 0 failed**；`npm run check:i18n`（web）**1377 键 / 11 域**对称。

## 2026-09-24（建立初始修订并验证生成成果可打开，PLAN-DM-036 Task 7）

- 新增 `src/dst_manager/application/creation_registration.py`（387 行）：`CreationRegistrationOperations` 把已发布的新项目接管为**普通工作区**并登记**初始修订**。`finish_creation(published: PublishedProject, *, worker_id=None, attempt=None)` 是 Worker 路径的唯一出口，`resume_creation_registration(job_id, payload)` 是启动恢复的幂等补登记入口（只消费持久成果投影，绝不回到暂存区重新生成成果）。登记顺序固定为「已发布 DST + 发布事务归档的 `manifest.json` 必须齐全 → `open_workspace(dst_path, workspace_root=target_dir)`（复用既有工作区 ID 推导、修订哈希与 `workspaces` 行写入）→ `resolve_workspace_standard` 物化项目内标准快照（复用既有恢复路径，库中版本缺失只降级为 `STANDARD_MISSING` 诊断、不阻断）→ `write_workspace_metadata` → `Database.add_revision`（`kind="creation"`、`source_json` 记录标准身份与本次生效的编号配置）→ `get_workspace` 重新打开复核」。对外成功响应（`workspace_id`/`revision_id`/`dst_path`）只在文件、SQLite、修订清单一致且工作区能重新打开之后产生；修订 ID 与已发布 DST 的内容哈希同值（与 `workspaces.current_revision` 同一口径），初始修订的 `before_hash` 记为空串（列非空，没有前序 DST），修订清单复用发布事务已归档的同一份 `manifest.json`，不另立格式。
- 登记失败语义（不放宽）：任何一步失败都保留已发布文件、发布日志与修订证据，任务落 `NEEDS_REVIEW` + `CREATION_REGISTRATION_FAILED`（绝不落普通 `SUCCEEDED`），`CreationRegistration` 返回 `workspace_id`/`revision_id` 为空但给出已发布 `dst_path` 供人工核对；`worker_id`/`attempt` 给出时终态写入走任务租约围栏（租约丢失只读回现场，绝不覆盖新主人状态）。幂等性逐项落实：工作区行 upsert、标准快照已存在即复用、同 ID 同身份的修订跳过（身份不一致以 `CREATION_REGISTRATION_FAILED` 拒绝覆盖）、任务闭环幂等。
- `application/creation_job.py`（594→608 行）：`CreationJobRunner` 新增可选 `registration`（登记回调协议），发布成功后**先登记、再落终态**（生产路径恒注入；未注入的隔离暂存/发布单测仍只闭环任务本身，`jobs.workspace_id` 保持 NULL）；`application/creation_execution.py`（201→213 行）在 `run_creation_job` 装配 `registration=self`，并把启动恢复的已提交分支从「只闭环成功」改为 `_resume_committed_creation`（已登记完成则跳过，否则按持久投影幂等补登记，补登记失败仍落 `NEEDS_REVIEW`）。`application/service.py` 只加 mixin 装配（363→385 行）；`infrastructure/persistence/database.py` 的 `finalize_creation_job` 追加 `workspace_id`/`revision_id` 关键字（同时写 `jobs.workspace_id` 与 `payload.workspace` 的 `{id, revision_id, dst_path}`，两者必须同时给出或同时省略）。
- 未新增写入格式：项目内标准快照仍由既有 `StandardOperations._restore_snapshot` 复制到 `.dst-manager/standards/<id>/<version>/`，工作区元数据仍由既有 `write_workspace_metadata` 原子写入 `.dst-manager/workspace.json`（`infrastructure/filesystem/workspace.py` 无需改动，保持既有字段与写法）。新稳定码 `CREATION_REGISTRATION_FAILED` 同批登记到 `message_catalog.py` 与 `web/src/i18n/locales/{zh-CN,en-US}/errors.ts`（`errors.creation.registrationFailed`）。
- 测试：新增 `tests/integration/test_created_project_opens.py`（641 行，**13 例**）——brief Step 1 两个逐字用例（创建成果作为普通工作区重新打开：标准身份、`workspace.revision_id == completed_creation.revision_id`、`.dst-manager/standards/szmedi.gas/2.1.0` 存在；登记失败 `NEEDS_REVIEW` 且 `workspace_id is None`），以及成功响应三字段与 `payload.workspace` 一致、初始修订与已发布 DST/编号配置/修订清单一致、逐组主 DWG 引用与 Handle、库中版本删除后仍由项目快照恢复语义、发布与登记之间库版本消失时登记仍成功且工作区照常打开、登记失败保留成果并可由重启**幂等补登记**（成果文件哈希与修订行数不变、绝不重新生成或覆盖 DWG）、重复登记幂等、修订清单缺失时拒绝报成功；测试不启动 AutoCAD（替身执行器接管 CAD，其余走真实服务链）。新增 `tests/system_autocad/test_creation_workflow.py`（276 行，**2 例**，参数化 2016/2020）：真实 Core Console + 匹配版本插件走完整创建链，逐组主 DWG 真实回读布局集合与 Handle 必须与 DST 记录逐项一致、DST 引用解析到本次生成的 DWG 且 `validate() == []`、重新打开后标准绑定/快照/修订齐备，并在测试前后核对私有样本模板哈希不变；缺环境/缺样本/缺插件按原因跳过，官方图纸集管理器（SSM）界面打开性无法在 Core Console 内自动化，已在模块文档中列为人工验收项、不记为已通过。`tests/integration/test_creation_job.py` 更新 1 例（启动恢复的已提交分支现在同时补登记，断言 `jobs.workspace_id`、`payload.workspace.revision_id` 与重新打开一致），另补模块文档说明 Task 7 语义。
- 验证：`uv run ruff check .` 通过；`uv run pytest` **1884 passed / 74 skipped / 0 failed**（收集 1958 例；本节原记「1957 passed」有误，1958 是收集总数而非通过数，已由 Task 7 审查修复轮复核更正）；`DST_MANAGER_RUN_AUTOCAD=1 uv run pytest tests/system_autocad/test_creation_workflow.py -q` **2 passed**（2020 16.45s / 2016 12.19s，真实 AutoCAD 2016 + 2020 Core Console 与插件）。
...
- 最小修法（不改变公共契约、不放宽回滚语义）：新增 `_read_creation_journal`（`except (OSError, json.JSONDecodeError)` → `None`，非 JSON 对象也返回 `None`）与 `_committed_creation_payload`（`revision_dir` 必须是字符串，成果投影的 `KeyError`/`TypeError`/`StopIteration`/`OSError` 一律返回 `None`）。循环中不可读取的日志直接 `continue`（跳过该日志、现场与日志原样保留），已提交但结构不可信的日志同样 `continue`——既不闭环成功，也不清理或覆盖任何成果文件；身份可解析但 `operation_id`/`attempt` 被改写仍走既有 `PUBLISH_MANIFEST_IMMUTABLE_MISMATCH` 隔离分支，未改宽。被跳过的任务由既有 `recover_stale_jobs` 落 `NEEDS_REVIEW` + `PUBLISH_JOURNAL_REVIEW_REQUIRED`，与 `_list_committed_operations_locked` 的既有等价容错一致。
- 测试：`tests/unit/test_project_publisher.py` 追加 **14 例**（截断 JSON/非 JSON/JSON 数组/JSON 字符串四种损坏形态下跳过且同批健康日志照常 `ROLLED_BACK`、日志路径不可读取（`OSError`）、已提交日志 7 种缺键/类型错误形态（`files`/`target`/`revision_dir` 缺失或类型错误、条目缺键、无 dst 条目）只跳过不动成果、回滚阶段日志缺键/类型错误落既有隔离分支 `NEEDS_REVIEW`），`tests/integration/test_creation_job.py` 追加 **3 例**（截断日志：服务仍能启动、损坏任务落 `PUBLISH_JOURNAL_REVIEW_REQUIRED`、同批另一任务照常 `ROLLED_BACK`、现场与日志保留；缺 `files` 键：落 `CREATION_PUBLISH_REVIEW_REQUIRED`；已提交但缺 `files`：不闭环成功、成果零改动、落 `PUBLISH_JOURNAL_REVIEW_REQUIRED`）。先 RED（`JSONDecodeError`/`AttributeError`/`KeyError` 从 `DstManagerService.__init__` 逃出）后 GREEN。
- 验证：`uv run ruff check .` 通过；`uv run pytest tests/unit/test_project_publisher.py tests/integration/test_creation_job.py` **86 passed**；`uv run pytest` **1843 passed / 72 skipped / 0 failed**（净新增 17 例）。

## 2026-09-24（建立新图纸集可恢复发布事务，PLAN-DM-036 Task 6）

- 新增 `src/dst_manager/infrastructure/filesystem/project_publisher.py`（474 行）与 `project_publish_types.py`（81 行）：`ProjectPublisher.publish_new_project(candidate, target, job_id, attempt) -> PublishedProject` 把 Task 5 的候选**可恢复地发布**到新项目目录。与既有 `RecoverablePublisher` 的差别只有语义：目标是**新建**目录（不存在或已存在但为空），因此没有 before 快照、没有工作区 `.dst-manager/` 命名空间——发布日志、修订证据与回滚隔离目录落在 Manager 应用数据目录的隔离 attempt（`<data_dir>/creation-jobs/<job_id>/attempt-NNN/`），目标项目目录只承载最终成果文件。**只按显式候选清单发布**（Task 5 复核发现候选 DST 先写盘后校验，暂存区可能残留校验失败的候选形文件，故绝不扫描 attempt 暂存区）。目标在发布前必须是「不存在」或「已存在且为空」，日志记录原状态、目标身份（已存在空目录）、候选清单/哈希、`job_id`/`attempt` 与每个已提交文件的 `result_hash`/`result_identity`；首次创建目录后只操作本次占有的精确路径。逐文件提交走「暂存副本 → 哈希复核 → 日志先落盘 → `move_no_replace` 原子换名 → 身份复核」，复用 `publish_primitives`（哈希/身份/原子换名）、`publish_journal`（同一 JSON 形态与原子写入）、`WorkspaceTransactionLock`（目标级锁）与 `publish_recovery`。失败只回滚本次已创建且**身份匹配**的文件（按身份换名到回滚隔离目录再删除，绝不就地按名字删），随后精确恢复「目录不存在」或「目录存在且为空」：`rmdir` 非空即失败、绝不递归删除；残余外部内容一律停止自动清理、保留日志并升级 `NEEDS_REVIEW`。故障注入作为显式构造参数（`fault_injector`），按 Interfaces 签名实现（brief Step 1 的模块级 `publish_candidate` 为示意调用）。
- `infrastructure/filesystem/publish_recovery.py`（248→377 行）：新增 `recover_creation_publishes(publisher, creation_jobs_root) -> list[CreationPublishOutcome]` 与 `creation_published_payload(journal)`。按 `creation-jobs/*/attempt-*/publish-journal.json` 枚举，先校验路径身份与日志的 `operation_id`/`attempt` 一致（不一致即以 `PUBLISH_MANIFEST_IMMUTABLE_MISMATCH` 终止，不做猜测性清理）；`COMMITTED` 只补齐证据归档、绝不重发或覆盖成果；`PREPARED`/`PUBLISHING`/回滚中断调用发布器的 `_rollback_creation`（与发布失败路径同一实现），结论为 `ROLLED_BACK` 或 `NEEDS_REVIEW`；每份日志都在**目标级发布事务锁**下处理（与发布路径同一把锁，锁路径记入日志），锁被占用说明同一目标上仍有进行中的发布或恢复，此时不介入现场。成果投影（`target_dir`/`dst_path`/`target_state`/逐文件 `sha256`/`size`/日志与修订证据路径）只有这一份实现，发布路径与启动恢复共用。
- `migrations/versions/0007_creation_jobs.py`（新，down_revision=`0006_dm020_extension_platform`）：`jobs.workspace_id` 改为可空（创建任务在普通工作区尚不存在时就要记录状态/进度/事件），新增稳定关联列 `creation_draft_id`；SQLite 走 batch 重建表，既有 `job_files`/`job_events`/`diagnostics` 行原样保留。`downgrade` 在仍有「无工作区」任务时显式以 `CREATION_JOBS_DOWNGRADE_BLOCKED` 阻断（不静默丢数据），否则恢复 `NOT NULL` 并删列。
- `infrastructure/persistence/database.py`（796→871 行）：`JobRow.workspace_id` 可空 + `creation_draft_id`；`LATEST_SCHEMA_REVISION=0007_creation_jobs`；`create_job(..., creation_draft_id=...)` 在 `workspace_id` 为空时不占普通工作区写锁；`_requires_cad` 统一「change_set+requires_cad / creation 都需要 CAD」，据此扩展 `claim_next_job`（创建任务可被 Worker 认领）、`recover_stale_jobs`（安全阶段重排、发布阶段落 `PUBLISH_JOURNAL_REVIEW_REQUIRED`）、`retry_job`（创建任务可重试，重排后下次认领 attempt 递增）；`_job_json` 暴露 `creation_draft_id`；`finalize_creation_job(job_id, published, worker_id, attempt)` 幂等写入 `payload.published` 并落 `SUCCEEDED`（不写 `workspace_id`）；所有 `WorkspaceWriteLockRow` 访问对 `workspace_id IS NULL` 显式跳过。
- `application/creation_job.py`（496→594 行）：`CreationJobRunner` 新增 `run(job_id, attempt, plan)`——按租约推进 `CAD_RUNNING(20) → VERIFYING(70) → PREPARED(80) → PUBLISHING(90)`，调用 `stage` 与 `publish_new_project`，成功后 `finalize_creation_job` 闭环；发布失败按现场结论映射终态（`ABORTED`→`FAILED`、`ROLLED_BACK`→`ROLLED_BACK`、`NEEDS_REVIEW`→`NEEDS_REVIEW`），暂存/计划故障落 `FAILED` 并写 `error_code`/`error_detail`，丢失租约以 `CREATION_JOB_LEASE_LOST` 拒绝覆盖新主人的现场。工作区登记（`workspace_id` 关联、标准快照与初始修订）留在发布成功之后的 Task 7，`jobs.workspace_id` 在发布前恒为 NULL。
- 新增 `application/creation_execution.py`（201 行，`CreationExecutionOperations`，与 `creation.py` 同层拆分以遵守容量契约）：`execute_creation(draft_id, preview_digest)` 只接收草稿 ID 与摘要，入队前**重新加载**标准、草稿、目标与设置/资产快照并重算摘要（漂移 409 `CREATION_PREVIEW_STALE`；阻断诊断 422 `CREATION_PLAN_INVALID`），创建任务以 `workspace_id=NULL`+`creation_draft_id` 入队；`reload_creation_plan` 供 Worker 侧执行前再次重算（漂移即拒绝）；`run_creation_job(job)` 装配 `CreationJobRunner` 并在标准包不可读取时落 `CREATION_STANDARD_MISSING`；`recover_interrupted_creation_jobs()` 在服务启动时按持久日志幂等闭环任务（`COMMITTED`→`SUCCEEDED`、`ROLLED_BACK`→`ROLLED_BACK`+`STARTUP_RECOVERY`、外部内容→`NEEDS_REVIEW`+`CREATION_PUBLISH_REVIEW_REQUIRED`），日志身份不可信时只按受控目录层级隔离任务。`application/creation.py` 把预览主体抽成 `_creation_preview`（一次重算同时返回草稿/计划/响应），`preview` 行为逐字不变。
- `interfaces/creation_contracts.py` 新增 `CreationExecuteRequest`（只含 `preview_digest`，夹带 `target_path` 等派生输出直接 422）；`interfaces/creation_api.py` 注册 `POST /api/creation-drafts/{draft_id}/execute`（返回 `JobResponse`）；`interfaces/responses.py` 的 `JobResponse.workspace_id` 改为可空并新增可选 `creation_draft_id`（创建任务没有普通工作区）；`application/service.py` 只加最小装配（一个基类、一个 `self.project_publisher`、一行启动恢复、`run_next_job` 三行创建任务分发，363→381 行）。
- 错误码登记：`message_catalog.py` 与中英文 `errors.ts` 新增 24 个创建码（`CREATION_PREVIEW_STALE`；Task 5 的 19 个：`CREATION_PLAN_INVALID`/`CREATION_JOB_ID_INVALID`/`CREATION_ATTEMPT_INVALID`/`CREATION_ASSET_PATH_INVALID`/`CREATION_ASSET_FILE_MISSING`/`CREATION_LAYOUT_TEMPLATE_MISMATCH`/`CREATION_LAYOUT_SET_MISMATCH`/`CREATION_LAYOUT_READ_FAILED`/`CREATION_LAYOUT_REFERENCE_INVALID`/`CREATION_HANDLE_INVALID`/`CREATION_HANDLE_PLACEHOLDER`/`CREATION_HANDLE_DUPLICATE`/`CREATION_HANDLE_SET_MISMATCH`/`CREATION_ROUNDTRIP_MISMATCH`/`CREATION_DST_INVALID`/`CREATION_SKELETON_INVALID`/`CREATION_SKELETON_UNREADABLE`/`CREATION_CAD_UNAVAILABLE`/`CREATION_CAD_FAILED`；本任务的发布事务 4 个：`CREATION_PUBLISH_INVALID`/`CREATION_PUBLISH_FAILED`/`CREATION_PUBLISH_CONFLICT`/`CREATION_PUBLISH_REVIEW_REQUIRED`），逐条核对 raise 点后登记，`tests/unit/test_message_catalog.py` 同步锁定清单（创建码 7→31）。新增中英文 `web/src/i18n/locales/*/creation.ts` 的 `diagnostic` 子域（39 个创建诊断码：18 个预览诊断 + 21 个 XLSX 导入诊断，与 `standards.diagnostic` 同一做法；定位信息由诊断对象自带，文案不带插值参数），并在 `web/src/i18n/index.ts` 注册该域（`check:i18n` 1375 键 / 11 域）。**注意：`creation.ts` 原本是 Task 8 的新建文件，本任务先建立 `diagnostic` 子域，Task 8 在该文件上追加界面文案键即可，不要重建。**
- 新增 `tests/unit/test_project_publisher.py` **39 例**（新文件）、`tests/integration/test_creation_job.py` 追加 **10 例**（20→30）、`tests/integration/test_creation_api.py` 追加 **5 例**并替换原「执行端点未暴露」1 例（32→36）、`tests/unit/test_database.py` 追加 **5 例**（27→32）（先 RED：`dst_manager.infrastructure.filesystem.project_publisher` 不存在、`jobs.creation_draft_id` 缺失、`CreationJobRunner.run` 不存在、`/execute` 路由缺失）。覆盖 brief Step 1 的三个逐字用例（`publish_failure_restores_original_target` 参数化 `target_existed` False/True、`test_nonempty_target_is_not_touched`、`test_execute_rejects_stale_preview` 409 + `CREATION_PREVIEW_STALE`），以及：显式清单发布（暂存区清单外文件不被发布）、日志逐字段断言、已存在空目录复用与身份记录、缺失父目录的创建与精确回收、**第一个/中间/最后一个文件**提交失败（各 3 例 × 目标两种原状态）与 `PREPARED`/`PUBLISHING` 中断（`fail_at_stage` / `interrupt_after_replace`，含提交 1/2/3 个文件后中断）的启动恢复、外部内容与身份替换一律 `NEEDS_REVIEW` 且不误删、恢复幂等、`COMMITTED` 恢复不重发成果、日志身份改写被拒、attempt 命名空间不可复用、候选身份/文件名门禁、创建任务在无工作区时的状态/进度/时间线/失败诊断、重试使用新 attempt 目录、服务启动恢复三条路径（回滚、`NEEDS_REVIEW`、已提交闭环）。
- 验证：`uv run ruff check .` 通过；`uv run pytest -q` **1826 passed / 72 skipped / 0 failed**（净新增 58 例，基线 1768 passed / 72 skipped 全通过）；`uv run python scripts/export_openapi.py` 后 `npm run check:api` 退出码 0；`npm run check:i18n`（1375 键 / 11 域）、`npm run check:ui`、`npx vitest run`（275 passed）与 `npm run build` 通过；`npx playwright test` **632 passed**。迁移在 `DST_MANAGER_DATABASE_URL` 指向的临时库上实跑两条路径：全新库 `uv run alembic upgrade head` → `0007_creation_jobs (head)`；既有 0006 库（含一条旧 `change_set` 任务行）升级后 `jobs.workspace_id` 可空、`creation_draft_id` 就位、旧行原样保留；迁移往返（head→0006→head）由 `tests/unit/test_database.py` 锁定。**未执行真实 AutoCAD 系统测试**（本任务不新增 CAD 命令，`tests/system_autocad` 仍按环境显式启用）。

## 2026-09-24（建立内置 DST 骨架与隔离候选生成，PLAN-DM-036 Task 5）

- 新增 `src/dst_manager/infrastructure/acsm_xml/assets/minimal_sheetset.xml`（62 行）：唯一受版本控制的权威 DST 骨架。静态结构、无示例业务值、无 AcSm ID；节点名/`clsid`/`vt` 与节点顺序按**实测**真实 AcSm 数据库核定（`sample/project1|2/图纸集数据文件.dst` 与仓库内黄金样本 `docs/shared/research/project1-dst-xml/`），只保留协议所需的类别标识与固定标记（`PromptForDwt`/`DwfType`/`PromptForName` = `-1`、`NewSheetLocation.Relative_FileName` = `.`），不复制示例名称/路径/实例 GUID、不执行 legacy PowerShell；legacy 清单里的 `AcSmSimpleFileReferece` 未出现在任何真实 DST 中，故不作为协议必需节点。文件末尾单个 `AcSmSubset`/`AcSmSheet` 是节点原型（形状与子节点顺序的权威来源），实例化时按计划克隆、原型本身不进入成果。
- 新增 `src/dst_manager/infrastructure/acsm_xml/creation.py`（420 行）：`build_minimal_acsm(plan) -> AcsmDocument` 用 DOM 写入全部动态值（禁止字符串替换 XML）：每次实例化生成新数据库指纹与全部对象 ID（ID 在最终树上一次性生成，克隆节点不复用原型 ID）；`AcSmSheetSet.Name` 取 `Path(plan.target_path).name`，项目路径写 `NewSheetLocation.FileName`；普通与派生属性按**属性名**（SPEC-DM-017 §3.2）写入并按作用域给 `Flags`（1=图纸集、2=图纸），保留属性写 `DSTManager.Standard`（`<standard_id>@<version>`）与 `DSTManager.StandardOptions`（版本化 JSON，键序排序固定）；布局引用写最终目标 DWG 的绝对/相对路径 + **受控占位 Handle**（16 位全 F，`is_placeholder_handle`/`pending_layout_handles` 是「尚未回填」的唯一判定口径——语义校验器拒绝 `"0"`，绝不用 `"0"` 伪装通过）。`apply_layout_handles` 回填真实 Handle 并要求覆盖一一对应、非占位/非零/十六进制/同 DWG 内数值唯一，回填后再复核一遍；写入 DST 的文本复用编辑域 `validate_xml_text`（同一错误码）。实例化后必须 `repair_report.status == "VALID"` 且 `validate() == []`，否则不返回文档。
- 新增 `src/dst_manager/infrastructure/autocad/worker.py` 的**固定请求**（+167 行，文件 385 行）：`LayoutCreationRequest`（构造时按 `encode_scr_argument` 同一套不安全字符判定校验，目标布局名不得重复）+ `ScriptRenderer.render_create_layouts`（删旧布局 → 逐张 `-LAYOUT _Template` 导入选中图幅并立即改名 → 改名到计划布局名 → 回读 Handle，与既有 `render_rebuild` 同一套已验证步骤，不接受自由参数）+ `LayoutCreationWorker`（`layout_names` 固定只读枚举、`create_layouts` 写 mbcs 脚本、执行、解析 sidecar 并拒绝零值 Handle；renderer/executor 可注入替身）。所有用户文本都经既有转义原语，绝不拼接路径/名称到 SCR。
- 新增 `src/dst_manager/application/creation_job.py`（496 行）与 `creation_candidate.py`（74 行）：`CreationJobRunner.stage(job_id, attempt, plan) -> CreationCandidate` 在 `<data_dir>/creation-jobs/<job_id>/attempt-<NNN>/{input,scripts,staging,logs}` 生成候选（DST、每组主 DWG、布局/Handle 清单、校验报告），**不写目标项目目录、不调用发布器**；作业 ID 走受控字符集门禁（越界片段拒绝）、阻断计划/空组/缺命名结果一律拒绝且不产生任何文件。逐组：从标准包受控基础模板复制主 DWG，布局模板复制进 attempt 后先只读枚举真实布局集合（`.dwg` 副本供 accoreconsole `/i`，保留原扩展名副本供 `-LAYOUT _Template`），声明图幅不在真实布局集合即 `CREATION_LAYOUT_TEMPLATE_MISMATCH` 整任务失败；主 DWG 实际布局集合与计划逐项不一致即 `CREATION_LAYOUT_SET_MISMATCH`；非法/占位/重复 Handle 拒绝。候选 DST 写盘后重新解码加载，复核修复状态与契约/XSD/语义诊断，再做 `project()` 与计划的逐项比对（图纸集名称、子集名、逐张图号/标题/布局名/绝对与相对 DWG 引用/Handle/普通与派生属性值、标准身份与编号配置保留属性）。任何一步失败只留隔离 attempt 日志，失败与重试都不产生半成品候选。
- 跨任务接口追加（用户裁决 Option A）：`domain/creation_plan_models.py` 新增冻结值类型 `CreationProperty`（`property_id`/AcSm 属性名/作用域）与 `CreationSettings`（标准身份 + 属性定义 + `numbering` + `suffix_options`），`CreationPlan` 新增字段 `settings`，由 `creation_planning._creation_settings` 填充。理由：执行链只拿到计划（`stage(job_id, attempt, plan)`），而 DST 必需的标准身份、AcSm 属性名（DST 按属性名匹配，`property_id` 与名称不同）与有效编号配置无法从逐张输出反推。**纯追加**：既有字段语义、`plan_digest` 输入集合、既有断言与诊断码逐字不变，Task 3 的 17 例计划测试全通过。
- 打包：`packaging/dst-manager.spec` 新增骨架目录 datas 条目（目标与 `creation.pyc` 同级），`tests/unit/test_packaging_spec.py` 登记 `infrastructure/acsm_xml/creation.py` 的 `Path(__file__)` 资源定位并新增静态守护（缺条目即 frozen 态新建图纸集崩溃）。
- 新增 `tests/unit/test_creation_acsm.py` **20 例**（先 RED：`dst_manager.infrastructure.acsm_xml.creation` 不存在）与 `tests/integration/test_creation_job.py` **20 例**（先 RED：`dst_manager.application.creation_job` 不存在），含 brief Step 1 的两个逐字用例（fresh ID 且与 legacy 占位无涉、`AcSmSheetSet.Name` 取目录名；每组一个主 DWG、Handle 总数 = 总张数）与 Step 5 的运行集合；另覆盖保留属性 JSON 逐字形态、按属性名写入、占位 Handle 显式可识别、非法/占位/重复/覆盖不全 Handle 拒绝、模板图幅不符与布局集合不符整任务失败、失败只留 attempt 日志且目标目录零写入、暂存区外零文件、重试新 attempt、DST 编码往返与 `project()` 逐项一致、XML 1.0 非法文本稳定拒绝。测试不启动 AutoCAD：注入模拟 Core Console 的替身执行器，真实跑通脚本渲染→写脚本→解析 sidecar 全路径。
- 验证：`uv run ruff check .` 通过；`uv run pytest tests/unit/test_creation_acsm.py tests/integration/test_creation_job.py tests/unit/test_acsm_contract.py -q` **52 passed**；`uv run pytest` **1768 passed / 72 skipped / 0 failed**（新增 41 例，基线 1727 例全通过）。**真实 CAD 冒烟（本机具备环境，已跑）：** 以真实模板（`sample/template/基础模板.dwg` 作基础模板、`市政项目模板-通用.dwg` 作布局模板，图幅 `A1`）副本在临时目录跑通 `stage`：AutoCAD 2020 与 2016 均得到 `repair=VALID / 语义诊断 0 / 2 个主 DWG / 3 张图纸`，真实回读的非零 Handle 与布局集合（`01 平面图`/`02 平面图`/`03 剖面图`）与计划逐项一致，候选 DST 可解码加载。**官方图纸集管理器验收仍未做**（打开候选 DST 核对子集/图号/标题/自定义属性属 Task 7 系统测试），本任务不以模拟测试宣称双版本资格通过。

## 2026-09-24（开放图纸集创建草稿与预览 API，PLAN-DM-036 Task 4）

- 新增 `application/creation_assets.py`（295 行）：受控标准资产解析与身份快照。`creation_standard_candidates` 只列已发布标准，并逐个给出是否可选与原因：依赖按扩展注册表清单校验（复用 `standard_dependency_gaps`，与发布门禁同一判定）、模板文件必须是包内声明路径下真实存在的文件（越界路径与缺失文件都按不可用）、且必须同时存在可用的基础模板候选与声明图幅的布局模板候选。`creation_asset_options` 生成与模板/导入共用的资产候选（标签取包内受控文件名，同类内文件名冲突时退回完整包内相对路径，保证同类内唯一）；`resolve_creation_assets` 额外按声明文件算 SHA-256 形成资产摘要；`standard_document_digest` 取已发布文档内容哈希。包内目录定位复用 `StandardStore.published_root/official_root`，文件路径安全复用 `StandardAssetOperations._asset_file`，不另立第二套规则。
- 新增 `application/creation.py`（426 行）：`CreationOperations` 以 mixin 组合进 `DstManagerService`，承载标准候选、XLSX 模板导出、全量导入与权威预览。预览重新加载标准、当前设置（编号 + 标题后缀 + 不编号关键字，取运行期快照）、草稿与目标状态，并校验资产文件与图幅声明；`preview_digest` 绑定标准文档哈希、草稿身份与修订、有效编号/后缀设置、资产哈希、目标状态（尚不存在 / 已存在空目录 / 形状非法）与领域计划 `digest` 及阻断诊断集合，任一项变化都产生不同摘要。目标只能是尚不存在的新目录或已存在的空目录，非空目录与同名文件都以 422 `CREATION_TARGET_NOT_EMPTY` 阻断，预览与执行都必须重查。空图纸组与资产文件缺失由预览入口显式给出阻断诊断（`CREATION_GROUPS_EMPTY`、`CREATION_ASSET_FILE_MISSING`），不让空组草稿通过。导入是原子全量覆盖：先整批解析（含标准身份与资产标签核对），最后一次性保存；解析失败返回完整可定位诊断且草稿 JSON 与修订号零变化，陈旧修订由保存门禁以 409 `CREATION_DRAFT_CONFLICT` 拒绝；工作簿字节上限与标准包单条目上限同口径（`CREATION_XLSX_SCALE_EXCEEDED`）。预览全过程不启动 CAD、不写文件。
- 新增 `interfaces/creation_contracts.py`（217 行）与 `interfaces/creation_api.py`（207 行）：注册 `GET /api/creation-drafts/standards`（先于身份路由注册，避免 `standards` 段被吞）、`POST/GET/PUT/DELETE /api/creation-drafts[/{draft_id}]`、`GET /{id}/xlsx-template`、`POST /{id}/xlsx-import`（multipart，`expected_revision` 为可选查询参数）、`POST /{id}/preview`。路由只做输入输出、状态码与错误映射：草稿阶段白名单复用领域 `CREATION_STEPS`，图纸组契约模型转领域输入值，导入拒绝返回统一错误负载 + 逐条 `diagnostics`（工作表/行/列）。`/{id}/execute` 不在本任务暴露（Task 6 接入），并由集成测试锁定路由表。`interfaces/api.py` 只加一行 `register_creation_routes(app)`。
- 错误码登记：`message_catalog.py` 与中英文 `errors.ts` 新增 `errors.creation` 域 7 个码（`CREATION_DRAFT_NOT_FOUND`/`CREATION_DRAFT_INVALID`/`CREATION_DRAFT_CONFLICT`/`CREATION_DRAFT_CORRUPT`/`CREATION_STANDARD_MISSING`/`CREATION_TARGET_NOT_EMPTY`/`CREATION_IMPORT_INVALID`，仅导入码带 `{count}` 插值参数），`tests/unit/test_message_catalog.py` 同步锁定清单。
- 依赖：新增运行时依赖 `python-multipart>=0.0.32`（FastAPI `UploadFile` 的必需依赖，`pyproject.toml` 与 `uv.lock` 同步更新）。
- 新增 `tests/integration/test_creation_api.py` **32 例**（先 RED：创建路由不存在，`POST /api/creation-drafts` 返回 405、预览/候选/模板端点 404），覆盖 brief Step 1 的三个逐字用例（草稿编辑后摘要变化、非法导入后 GET 草稿逐字相同、非空目标 422 `CREATION_TARGET_NOT_EMPTY`），以及草稿 CRUD 与乐观修订、标准候选可用性与原因（缺模板文件、缺受信依赖、同类标签冲突）、模板导出往返、全量导入成功/陈旧修订/超限/不可读、导入拒绝诊断定位、按组表格与逐张属性明细、摘要对标准文档/设置/资产/目标状态/草稿的敏感性、空目录目标合法且摘要不同、同名文件阻断、空组阻断、缺失标准 404、派生与未知输入拒绝、预览不改草稿、预览不启动 CAD（替换 `CoreConsoleExecutor.run` 与 `get_layout_names` 为失败）与执行端点未暴露。
- 验证：`uv run ruff check .` 通过；`uv run pytest tests/integration/test_creation_api.py -q` **32 passed**；`uv run pytest` **1727 passed / 72 skipped / 0 failed**（新增 32 例，基线 1695 例全通过）；`uv run python scripts/export_openapi.py` 后 `npm run check:api` 退出码 0（新增 6 个路径、17 个 schema，既有路径与 schema 逐字不变）；`npm run check:i18n` 通过（1312 键 / 10 域）。

## 2026-09-23（修复创建求值缺键门禁并生成确定性创建计划，PLAN-DM-036 Task 3）

- 修复 F2：`domain/standard_rules.py` 原先把**漏传普通属性键**当空值（`result.get(id, "")`），宿主少传一个键会被静默求值成空串。现在映射源读取处与组合属性的普通属性令牌处都先判断键是否存在于输入 `values`：缺键报 `STANDARD_DERIVED_UPSTREAM_INVALID`、该派生属性进入阻断集合、**不写派生键**并阻断下游组合；显式 `""` 保持「可选源为空 → 空结果」的既有语义；枚举合法性与标准发布门禁（`publish_diagnostics`）未改，调用方行为只收紧不改变。
- 新增 `domain/sheet_numbering.py`：把 `derive_document_structure` 中与既有 DOM 无关的连续编号/不编号原语抽为同层纯函数——`format_number_range`（图号范围单值/首末值）、`number_seed`（起点与位数取首个纯数字图号，不编号组的 0 填充必须由调用方先排除）、`is_unnumbered_title`（复用 `title_matches_keywords`）、`compress_group_title`（标题紧凑范围）。`domain/editing.py` 改为共同调用这些原语，错误码与文案逐字不变（空子集仍抛 `EditingError("EMPTY_SUBSET", ...)`），文件净减 25 行。
- 新增创建计划领域模块：`domain/creation_planning.py`（入口 `create_creation_plan` 与逐组投影）、`domain/creation_plan_models.py`（计划对外形状与确定性摘要）、`domain/creation_plan_inputs.py`（输入门禁与标准资产解析）。按容量契约（单文件约 500 行软上限）拆为三个同层模块（420/189/191 行），计划给定的单文件入口 `dst_manager.domain.creation_planning` 再导出全部公共名字。
- 计划形状：`CreationPlan.has_errors`/`diagnostics`（每条带 `group_id`/`property_id`）/`target_path`（预检 ruling：Task 5 用 `Path(plan.target_path).name` 写入 `AcSmSheetSet.Name`）/`digest`/`sheetset_values` 与逐组 `GroupPlan`（`number_range`、`title_range`、`base_template`、`layout_template`、`paper_layout`、`dwg_name`、`target_path`、`sheets`、`property_cells`）；每张 `SheetPlan` 带 `number`/`title`/`layout_name` 与完整普通 + 派生属性值，`property_cells[property_id]` 带 `first_value` 与逐张 `sheets` 明细（每行 `number`/`value`）。计划器不读文件系统、不启动 CAD、不查标准包文件：资产只按 `asset_id` 与图幅角色解析出包内相对路径。
- 关键契约：`sheetset` 与每组 `sheet` 输入必须是完整 `property_id` 字典，缺键报 `CREATION_SHEETSET_VALUE_MISSING`/`CREATION_GROUP_VALUE_MISSING`，绝不在求值时临时补空串、绝不回填旧派生值；必填非空（`CREATION_REQUIRED_VALUE_MISSING`）、枚举非法、组图名空/重复（忽略大小写与首尾空格）、张数非法、资产/图幅声明非法、`DUPLICATE_LAYOUT_NAME`、`DWG_TARGET_COLLISION`（大小写不敏感）均诊断且计划不可执行。图号位数/起点取 `standard.numbering`，标题后缀与不编号关键字取 `SuffixOptions`；不编号组全组图号为补零 `0` 且不占号，后续组继续顺延；`sheetset` 派生失败或目标重名时不产出半成品文件名（`dwg_name`/`target_path` 为空）。`digest` 用稳定 JSON 序列化覆盖标准身份、草稿身份与修订、有效编号配置与全部派生输出。
- 新增 `tests/unit/test_creation_planning.py` **17 例**（先 RED：`dst_manager.domain.creation_planning` 不存在；缺键用例在修复前把 `prop-code` 求值成 `""`），`tests/unit/test_standard_rules.py` 新增 **2 例**（先 RED：缺键被当空值、组合令牌缺键被按空串拼接）；含 brief Step 1/Step 5 的四个逐字用例（必填清空、不编号组 `00` 不占号、按组预览首值与全量明细）。
- 验证：`uv run ruff check .` 通过；`uv run pytest tests/unit/test_standard_rules.py tests/unit/test_creation_planning.py tests/unit/test_unnumbered_subsets.py tests/unit/test_v021_editing.py tests/unit/test_standard_naming.py` **130 passed**（现有编辑结果逐字保持）；`uv run pytest` **1695 passed / 72 skipped / 0 failed**（新增 19 例，基线 1676 例全通过）。
...
- `useStartNavigation` 新增一次性进入意图 `StandardsEntryIntent`（`browse` / `import-package`）：`openStandards(intent?)` 记录意图，`goWelcome()` 清除意图；`App.vue` 只装配事件，创建进既有明确不可用占位、管理进标准库、导入进标准库并直接打开**同一个**既有导入对话框（不新增第二套导入表单或导入状态）。
- 新增中英文同构文案键 6 个（欢迎页总标题/说明、其他任务标题/说明、创建任务名、标准边界说明），`check:i18n` 1299 键 / 10 域通过。
- 验证：`useStartNavigation` 单元测试 3 项通过（新增意图用例先 RED 后 GREEN）、`check:i18n` 与 `check:ui` 退出码 0、`standards-welcome.spec.ts` **40 passed / 0 failed**（新增“打开优先双栏且三个次级任务走既有去向”与“900×768 单列且打开任务仍排在最前”两例）、`npm run build` 退出码 0。

## 2026-09-23（编制图纸标准平台欢迎页与编辑器视觉收口计划）

- 新增 `PLAN-DM-039`（proposed）：承接 PLAN-DM-035 欢迎页双栏未落地与 PLAN-DM-038 编辑器仍嵌在标准库右栏的页面级偏差；计划以失败结构 E2E 锁定欢迎页唯一主动作/三个次级入口、标准库与编辑器互斥页面状态、返回选择与 dirty 门禁，再实现约 2:1 欢迎页和独立全宽六分区工作台。视觉关闭改为“候选证据 → 用户对照 SPEC-DM-016/017 Demo 裁决 → 晋升 G4/G8 → 真实 WebView2 100/125/150/200% 检查”，不再以同一生产页面抓两遍一致代替设计验收；范围明确不修改标准 Schema、后端 API 或 PLAN-DM-036 创建流程。

## 2026-09-23（移除误提交的构建缓存并合并 PLAN-DM-038 分支）

- 取消跟踪上一提交误纳入的 `node_modules/.vite/vitest/.../results.json` 构建缓存，并在 `.gitignore` 补充根级 `/node_modules/` 规则，避免仓库根目录再次纳入依赖缓存。验证：Ruff 通过，全量 Python 测试 1628 项通过 / 72 项跳过，Web 单元测试 274 项、生产构建与 Playwright 端到端 620 项均通过；仅清理缓存与忽略规则，未改动产品代码。

## 2026-09-23（完善 PLAN-DM-036 标准驱动创建实施计划）

- 用户确认 SPEC-DM-018 四阶段 UI、Manager 内置受控 DST 骨架，以及以最终项目目录名写入 `AcSmSheetSet.Name`（独立于自定义项目属性）；同步修订 RFC-INT-003 骨架归属、SPEC-DM-018 状态和文档索引。重写 PLAN-DM-036 的按组输入、双工作表 XLSX 全量导入、必填实际值/F2 缺键门禁、不编号与按组预览、骨架验证、隔离生成、新目录事务、工作区接管及前端验收任务；F3/F4 独立小修，F5 登记为独立 Todo 且不并入本计划。仅更新文档，未实施产品代码。

## 2026-09-23（新建图纸集 Demo 图纸组操作列对齐）

- 图纸组表末列改为「操作」，排序与删除按钮改为与图纸目录插件一致的 `↑`、`↓`、`✕` 图标和 32×32 样式；保留原有行为、首末行禁用状态和逐组可访问名称，新增渲染回归测试。验证：Demo 测试 28 项、Ruff、相关 Python 测试 56 项通过；全量 Python 测试仍有未改动 `test_setup_bat.py` 的 2 项中文输出断言失败。

## 2026-09-23（标准驱动新建图纸集交互 Demo）

- 新增 SPEC-DM-018 独立 HTML/CSS/JS 交互 Demo：串联标准选择、项目属性与路径、图纸组复制/排序/批量修改、按组预览、逐张属性模态、编号设置失效与冲突阻断、XLSX 双表结构及全量覆盖提醒；所有数据与创建结果均为模拟，不写工程文件。同步规范和文档索引入口，新增 4 项确定性预览测试与[设计 QA 记录](.planning/memos/dst-manager/2026-09-23-spec-dm-018-demo-design-qa.md)。Demo 测试 27 项、Ruff、相关 Python 测试 56 项通过；全量 Python 测试另有未改动 `test_setup_bat.py` 的 2 项中文输出断言失败。

## 2026-09-23（标准驱动新建图纸集 UI 设计草稿）
...
- 实现标准派生属性编译、求值与发布诊断（PLAN-DM-038 Task 2）：重写 `domain/standard_rules.py`，删除旧自由规则拓扑，改为固定拓扑「普通属性 → 映射属性 → 组合属性」；映射行集合由源枚举列表决定（顺序与 `enum_item_id` 都跟随枚举项，存储映射表只提供目标值），枚举改名后按 `enum_item_id` 保留目标并进入待确认 warning；可选源为空得到空结果，非空源值非法/映射未覆盖/上游失败时下游组合标记无法计算且不写入部分结果，调用方传入的派生旧值一律丢弃；新增 `publish_diagnostics` 静态门禁（源唯一占用、目标为空阻断，重复目标合法，确认快照不一致 warning）。单元测试 11 项通过，Ruff 通过。
- 实现标准级唯一 DWG 命名模板（PLAN-DM-038 Task 3）：新增 `domain/standard_naming.py`，提供 `render_dwg_filename`/`validate_dwg_filenames`/`publish_naming_diagnostics`。只允许 `subset.scope/name/sequence` 与 sheetset 属性（sheet 属性令牌在渲染层也拒绝），`subset.sequence` 支持宽度 1–12 的受控补零，`.dwg` 由系统固定追加；文件名主体非空、不得自带扩展名、不含路径片段/控制字符/Windows 非法字符、不以空格或句点结尾、不是保留设备名、含扩展名不超 240 字符，且不做任何字符替换；`validate_dwg_filenames` 按 Windows 大小写不敏感语义对可用结果做碰撞预检并标记 `DWG_TARGET_COLLISION`；模板同时缺少 `subset.scope` 与 `subset.sequence` 时发布 warning `DWG_NAMING_UNIQUENESS_UNPROVEN`。单元测试 23 项通过，Ruff 通过。
- 接线标准草稿与发布门禁（PLAN-DM-038 Task 4）：仓储层拆分两阶段门禁——`create_draft`/`save_draft` 只过结构解析（允许空映射目标等语义未完成内容），`publish`/`get`/包导入过完整发布解析，并在 `publish`/`import_package` 保留一道不可绕过的发布门禁防线（首个 error 以同码前缀转为仓储错误，草稿目录不移动）。应用层发布与导入在目录移动前汇总 `publish_diagnostics` + `publish_naming_diagnostics`，首个 error 转 422，warning 随成功响应返回（`StandardPublishResponse.diagnostics`，`StandardDiagnosticModel` 新增 `property_id`/`segment_index` 定位字段）。DST 提取改为生成稳定 `property_id`（规范化名称 sha256 前 12 位）、`kind="text"`、空历史名称，写入默认 `dwg_naming`（`subset.scope + " " + subset.name`）且不推测现有 DWG 文件名前缀；同名跨作用域按全局唯一规则拒绝（AcSm DOM 层已先行拒绝）。重建全部标准域测试夹具为新 Schema，新增旧 `rules` 结构以明确 Schema 错误拒绝、草稿可保存但不可发布、包导入同样过门禁、DST 默认命名与稳定 ID 等回归用例。
- 重建标准前端草稿模型（PLAN-DM-038 Task 5）：`features/standards/draftModel.ts` 删除 `DraftRule`/`ORDINARY_RULE_KINDS`/`COMPOSITION_RULE_KINDS` 与循环拓扑逻辑，改为 `DraftProperty` 判别联合（文本/枚举/映射/组合）+ `DraftSegment` + `DraftDwgNaming`，新增 `referencesTo`（映射源/组合令牌/DWG 命名令牌的反向引用，供删除保护）、两段门禁 `draftDiagnostics`（只含结构致命错误，草稿可保存语义未完成内容）与 `publishIssues`（结构 + 语义 error/warning，与后端同码）、字段范围 `compositionFields`/`dwgNamingFields`、示例预览 `renderDwgNamingPreview`/`renderCompositionPreview`（默认值→枚举首项→首个映射目标→占位；模板骨架做与取值无关的文件名安全检查）以及只产生普通属性的 CSV 导入；`EDITOR_SECTIONS` 改为六分区。`publishModel.ts` 重接为新诊断到六分区/定位目标（属性行、映射行、命名令牌）的映射，资产引用改为枚举项/映射目标/组合与命名固定文本。新增/重写单测 27 项（旧模型下 25 项失败，新模型全通过），前端全量单测 221 项通过。
- 实现标准普通属性表与枚举编辑模态框（PLAN-DM-038 Task 6）：新增 `OrdinaryPropertyEditor.vue`（八列表格：属性名/作用域/类型/必填/默认值/枚举值/说明/删除；文本行的枚举摘要灰显不可聚焦；文本↔枚举在普通属性内可切换；行级发布诊断标记；CSV 只创建普通属性）与 `EnumValuesDialog.vue`（列表自上而下即枚举顺序、顺序列只显示序号与可访问命名的上移/下移、内部 `enum_item_id` 不外显、取消丢弃本轮增删改排序、保存一次性提交、宽度与最大高度施加于外层对话框且底部操作栏固定可见）。删除旧的 `StandardPropertyEditor.vue` 与 `StandardRulesEditor.vue`。新增 `--checkbox-size:16px` 令牌：必填复选框本体固定 16×16，点击区域由标签扩到 32×32，不继承输入框统一高度。新增中英文语言包键（六分区、普通属性、枚举模态与全部新诊断码）并补齐 `standards.diagnostic.*`。组件单测 14 项通过，前端全量单测 235 项通过，check:i18n（1322 键）与 check:ui 通过。
- 实现标准派生属性表与映射编辑模态框（PLAN-DM-038 Task 7）：新增 `DerivedPropertyEditor.vue`（七列表格：属性名/作用域/源属性摘要/类型/说明/编辑/删除；映射↔组合可在派生属性内切换并清空载荷；源属性摘要显示源属性名与「待确认」；删除同样受引用保护并向上抛出引用列表）与 `MappingPropertyDialog.vue`（源属性选择器按作用域与唯一占用过滤、当前已选源以禁用项保留；映射行由源枚举列表固定生成、源值只读、不提供行增删与排序；确认把当前有序 `(enum_item_id, 显示值)` 写入确认快照；界面不展示内部 ID）。删除旧的 `MappingTableEditor.vue`。`draftModel.ts` 抽出 `mappingRows`/`mappingConfirmationRequired`/`selectableMappingSources` 纯函数并复用于发布诊断。组件单测 11 项通过，前端全量单测 246 项通过，check:i18n（1335 键）与 check:ui 通过。
- 统一组合属性与 DWG 命名编辑（PLAN-DM-038 Task 8）：新增共享原语 `TokenExpressionEditor.vue`（左字段浏览器 + 右单行令牌编辑框 + 下示例预览；字段点击插入不可拆分令牌、固定文字直接输入、方向键与退格按令牌整体移动/删除、用户输入的 `{...}` 一律当固定文本、序列化始终是结构化 `DraftSegment[]`）与 `features/standards/tokenFields.ts`（字段清单纯函数：分组、显示名与示例取样，不重复实现可见性规则）；新增 `CompositionPropertyDialog.vue`（独立令牌缓冲、取消丢弃、保存写回、底部操作栏固定可见）与 `DwgNamingEditor.vue`（字段只有三个 subset 系统字段与全部 sheetset 属性、`.dwg` 在编辑框外固定追加、示例预览明确标注、模板缺少 `subset.scope`/`subset.sequence` 时重名风险 warning）；`DerivedPropertyEditor` 接线组合模态框。删除旧的 `CompositionEditor.vue`。新增中英文语言包键（token/composition/naming，共 1347 键）。单测 12 项通过，前端全量单测 260 项通过，check:i18n 与 check:ui 通过。
- 重接标准编辑器、发布检查、语言包与 E2E（PLAN-DM-038 Task 9）：`StandardEditor.vue` 改为六分区（基本信息/普通属性/派生属性/DWG 命名/模板资产/版本与发布），保存门禁只取结构致命错误（`draftDiagnostics`），发布检查取全部 error/warning（`publishIssues`），诊断跳转按归属分区并聚焦属性行、直接打开派生模态框或命名令牌；`StandardPublishReview.vue` 改为六检查域并按属性名/源枚举显示值/令牌序号定位（不显示内部 ID）；`StandardSectionNav.vue` 补 `data-testid`；`DwgNamingEditor` 支持跳转聚焦。语言包删除旧 `properties`/`rules` 域与旧通用规则诊断码文案，能力摘要改为普通/派生属性计数（1292 键）。E2E 夹具重建为新 Schema（`prop-label` 改为 sheet 组合，`partialMappingDraft` 同时含空目标 error 与待确认 warning），标准编辑器规格重写为 SPEC-DM-017 场景集（单向流程、枚举改名与排序、取消不落盘、内部 ID 不外显、跨作用域重名、引用删除阻断、源唯一占用、字段范围、未完成草稿保存、发布 error/warning 区分与模态跳转、DWG 非法名、紧凑复选框、900×768 与 200% 模态操作栏、纯键盘令牌与焦点归还），视觉证据规格改抓六分区状态（`g4-03-ordinary`/`g4-04-derived`/`g4-05-dwg-naming`/`g4-12-dwg-naming-narrow`）。前端全量门禁：单测 260 项、check:i18n、check:ui、`npm run build` 与全量 Playwright 619 项全部通过。
- 收口标准属性与命名整改（PLAN-DM-038 Task 10）：静态搜索确认生产代码、测试与夹具零旧模型残留（`StandardRule`/`DraftRule`/`ORDINARY_RULE_KINDS`/`COMPOSITION_RULE_KINDS`/`rule_id`/顶层 `rules`/`STANDARD_RULE_*` 全部为 0 命中，仅 `domain/standards.py` 一句说明旧模型已被替换的文档字符串命中）；`web/src/api/openapi.json` 与 `schema.d.ts` 按后端契约重新生成（发布响应新增 `diagnostics`，诊断模型新增 `property_id`/`segment_index`）。文档同步：PLAN-DM-036 声明必须先完成 PLAN-DM-038 并列出消费的 `compile_standard_properties`/`evaluate_standard_properties`/`render_dwg_filename`/`validate_dwg_filenames`；GUIDE-DM-007 全面改用普通/派生属性与唯一 DWG 模板术语；`docs/dst-manager/README.md`、计划索引与 SPEC-DM-016 证据 README（六分区替换说明）同步；PLAN-DM-038 标记 completed 并追加实际验证摘要。全量门禁：ruff、`pytest` 1697 passed / 72 skipped / 0 failed、`uv lock --check`、check:api、check:i18n（1292 键）、check:ui、test:unit 260、`npm run build`、Playwright 619 passed / 0 failed。
- 整分支复核修复轮（PLAN-DM-038 收口后）：修掉 2 项阻断缺陷与 2 项语义不一致——① 新建空白标准仍写入已删除的顶层 `rules` 且缺少 SPEC §6.3 要求的默认 DWG 命名模板，导致新建草稿一进编辑器就被结构门禁拦住无法保存；现由 `draftModel.blankStandardDocument()` 统一生成 Schema v1 文档（无 `rules`、含默认 `dwg_naming`），并补单元与 E2E 断言。② 应用层发布草稿未捕获 Schema 语义错误，`STANDARD_PROPERTY_NAME_INVALID` 等「草稿可存、发布必拒」的输入会返回 500 而非稳定 422；现以 `_store_error` 转 422 并补 HTTP 用例。③ 属性作用域在表格内可一键直接修改（违反 SPEC §3.2「作用域创建后不可直接修改」），现既有属性改为只读徽标、仅本次新建行可选作用域（普通与派生表一致）。④ 映射属性缺源在后端被静默接受（永不求值的映射属性可通过发布），现后端发布语义层要求映射必须有源、前端把「缺源」移入发布门禁（悬空 ID 仍在结构门禁），前后端口径一致；同时后端空名判定改用规范化名称，避免两个仅含空白字符的同名属性同时发布。另修：文件名长度按码点计数（与后端 `len()` 同口径，消除星平面字符下的前后端分歧）、删除死键 `standards.token.literal`、修正 SPEC-DM-016 证据索引与计划残余风险表述。修复轮全量门禁：ruff、`pytest` 1700 passed / 72 skipped / 0 failed、test:unit 266、check:i18n（1293 键）、check:ui、`npm run build`、Playwright 620 passed / 0 failed。
- 过渡状态（本任务已知且由后续任务收口）：`vue-tsc` 报 97 个类型错误，集中在 Task 6～9 即将删除或重写的 6 个组件（StandardEditor/CompositionEditor/MappingTableEditor/StandardRulesEditor/StandardPropertyEditor/StandardPublishReview），Task 9 Step 6 恢复全绿。
- 过渡状态解除：后端全量门禁恢复全绿（`pytest` 1697 passed / 0 failed / 72 skipped）。
...
- 新增 `SPEC-DM-016-welcome-demo.html` 单文件欢迎页交互 Demo：按“打开 DST 优先”重构为约 2:1 双栏，主栏覆盖点击选择、拖放、无桌面壳路径输入、文件类型错误与模拟打开状态；辅栏按固定顺序提供从标准创建新图纸集、管理图纸标准、导入 `.dststandard` 包三项次级任务，明确 PLAN-DM-036 尚未实施且不存在无标准创建回退，并在 900×768 降为主任务优先的单列。新增独立模型测试 4 项，锁定唯一主动作、文件类型边界、标准选择起点与空最近记录行为。
- 新增 `SPEC-DM-017-standard-properties-and-dwg-naming-demo.html` 单文件交互 Demo，作为 `PLAN-DM-038` 实施对照：以六分区标准编辑器串联普通属性、派生属性与全局 DWG 命名，覆盖文本/枚举联动、枚举扩展对话框、映射源唯一性与固定映射行、组合属性令牌编辑与预览、命名字段边界、默认模板及缺少顺序/范围时的重名 warning；同时保留模板资产和版本发布分区与 PLAN-DM-035 的既有职责边界。Demo 使用去敏模拟数据，不调用 API、不写入工程文件。
- 新增 `PLAN-DM-038`（proposed），把 SPEC-DM-017 拆为 10 个可独立验证任务：Schema v1 属性模型、映射/组合求值、全局 DWG 命名、草稿/发布/包/DST 提取接线、前端草稿模型、普通属性、派生属性、共享令牌编辑器、六分区编辑器与全量收口；明确 PLAN-DM-036 必须在该计划完成后再实施。
- 新增并接受 `SPEC-DM-017`，将标准属性重新收敛为“普通属性/派生属性”两类：普通属性只含文本/枚举，派生属性只含映射/组合；明确全局属性名唯一、稳定内部 ID、枚举驱动映射表、作用域约束、组合字段边界、DST 文本物化和失败传播。DWG 命名拆为标准级唯一模板，只允许 `subset.scope/name/sequence` 与 sheetset 属性，采用与组合属性一致的字段浏览器/单行令牌/预览结构；标准创建不再使用隐式 DWG 前缀。因标准包尚未对外发布，当前 Schema v1 直接更新并同步重建开发期数据，不保留旧规则兼容层。同步标记 SPEC-DM-016 对应旧章节由新规范取代。
- 记录后续待办“绑定标准工作区的属性与 DWG 命名语义恢复”：区分普通图纸集与绑定标准图纸集；后者从精确标准版本恢复文本/枚举、映射/组合、只读及派生重算语义，标准包 ID 与 AcSm GUID 保持独立，并按“作用域 + 规范化属性名/历史名称”安全迁移且保留原值；同时恢复全局 DWG 命名模板的只读锁定、输入变化重算、冲突阻断及事务改名。当前仅登记设计边界，不改动运行时代码。
...
- 新增 6 个编辑器组件：`StandardEditor.vue` 壳层（顶部固定标准名称/草稿标识/保存状态/“保存草稿”/“发布检查”，左侧七个分区导航，右侧只渲染当前分区；结构诊断摘要可一键跳到对应分区与映射行；直接保存状态机遵循 SPEC-DM-015：clean 用可聚焦的 `aria-disabled`、dirty 可保存、dirty-invalid 优先显示错误并阻断、保存成功以服务端响应重建基准；未保存修改离开走三选一门禁）、`StandardSectionNav.vue`（原生按钮 + `aria-current`，不用伪 tab 角色）、`StandardPropertyEditor.vue`（属性表 + CSV 导入，子集作用域只显示预留说明且不出编辑控件）、`StandardRulesEditor.vue`（规则列表 + 侧边结构化编辑器 + 实时自然语言摘要，不提供 JSON/脚本框）、`MappingTableEditor.vue`（一条规则 = 源字段 + 目标字段 + 两列表，逐行增删、批量粘贴（整表设置）、行级诊断可点击并聚焦该行首个可编辑单元格）、`CompositionEditor.vue`（可排序片段、补零格式码、字段浏览器、示例结果与就地诊断）。模板资产与版本发布分区保留占位说明（Task 10 接手）。
- 接线：状态控制器新增 `draft`/`draftPending`/`draftError`/`loadDraft`（带代次保护）/`adoptDraft`/`closeDraft`，`StandardApi` 新增 `fetchDraft`；`StandardsView` 新增编辑器开关与保守护卫（切换选中项先过三选一），草稿保存走 `PUT /api/standards/{id}/{ver}`（`POST /api/standards/drafts` 对已存在草稿会 409）、创建草稿后直接进入编辑器；`StandardDetailPane` 去掉临时的 `editorAvailable` 禁用占位。**修复假 dirty**：编辑缓冲与基线必须经过同一规范化（否则补齐 `dependencies` 等默认键会让刚加载的草稿被误判为有未保存修改）。
- 共享 `UnsavedInputDialog` 增加可选的 `message`/`saveLabel` 覆盖（默认文案与行为不变），使直接保存流显示“保存草稿并离开”而不是草稿流的“加入草稿后继续”，三选一结构与门禁语义保持一致。
- i18n 补 `standards.editor/sections/properties/rules/mapping/composition/diagnostic/pendingSection` 文案（中英同构 1153 键）；`hardcoded-allowlist.json` 新增一条登记（属性 CSV 的布尔单元格别名“是”是用户输入值，不是界面文案）。
- E2E：新增 `standards-editor.spec.ts` 8 项（映射批量粘贴后保存为一条规则、分区切换不丢输入与改回回到 clean、子集属性只读说明、保存失败保留输入可重试、修订冲突不静默覆盖、未保存离开三选一门禁、错误摘要跳到映射行并聚焦、纯键盘操作）；`fixtures/standards.ts` 扩展草稿端点（GET 草稿、PUT 按身份保存、可注入保存失败）与 `draftDocument`/`partialMappingDraft` 夹具；`standards-library.spec.ts` 的草稿编辑入口用例改写为直接进入编辑器。
- 验证：`vue-tsc -b`、`check:api`、`check:i18n`、`check:ui`、`npm run build` 全过；全量单测 209 项通过；标准系 E2E 21 项加 settings-dialog 33 项、守卫相关（properties-buffer + i18n-workflows）45 项均通过。
