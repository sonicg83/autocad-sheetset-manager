...
- 预览纯模型与预览会话（`web/src/features/creation/previewModel.ts` + `previewSession.ts`，新增）：前者是无状态投影（`summarizeSheetValues`、`previewPropertyColumns`、`previewDiagnosticTarget`、`splitPreviewDiagnostics`），后者是「按需保存 → 请求权威预览 → 写入摘要与可执行标志 → 按摘要执行」的状态迁移。`store.ts` 只组合两者（478 → 507 行），对外新增 `preview()`/`execute()`/`invalidateForSettingsChange()`，`CreationApi` 新增 `previewDraft`/`executeDraft`（`api/creation.ts` 手写防御性映射，`schema.d.ts` 无漂移）。任何草稿输入、标准身份或编号设置变化都清空 `previewState`/`previewDigest`/`canExecute`；执行**只发送 `preview_digest`**，无有效预览时不发请求。
