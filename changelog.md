...
- 验证：`uv run pytest tests/unit/test_sheet_catalog_workbook.py -q` **24 passed**；`tests/unit` 全量 **903 passed, 6 skipped** 无回归；`uv run ruff check src/dst_manager/extensions/builtin/sheet_catalog tests/unit/test_sheet_catalog_workbook.py` 通过。已知边界：XLSX 无持久化空串，空字符串单元格落盘为空单元格（读回 `None`），写入侧跳过空值保持空单元格语义。
