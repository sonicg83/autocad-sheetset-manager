...
- **Task 5 全轮收口**：三轮评审闭环（首评 Needs fixes → 修复轮 1（Ruling 33）→ 二次评审通过 + 一项交裁定 → Ruling 35 → 修复轮 2）。例外表 **320 → 258**（−62 纯删除，与 Task 5 Files 1:1 重合，零误删）。**Task 5 至此关闭**，下一步 Task 6（图纸目录页；注意 `SheetPropertyEditor.vue:108` 同款 hover 规则——原语已就位，Task 6 **只需删页面规则**）。
