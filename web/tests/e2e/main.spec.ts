...
test("CSV 预览丢弃换文件和乱序响应并只导入冻结文本",async({page})=>{
  ...
  await openWorkspace(page);await page.getByRole("tab",{name:"属性"}).click();await page.getByRole("button",{name:"导入 / 导出"}).click();await page.getByRole("button",{name:"导入 CSV"}).click();const csvInput=page.getByLabel("属性 CSV 文件");const csv=(name:string)=>({name:`${name}.csv`,mimeType:"text/csv",buffer:Buffer.from(`type,name,default_value\nsheet,${name},\n`,"utf8")});
...
test("加载新工作区时隐藏旧编辑器并阻断跨工作区执行",async({page})=>{
  ...
  openB.resolve();await switching;await page.getByRole("tab",{name:"属性"}).click();await expect(page.getByLabel("图纸集名称", {exact: true})).toHaveValue("工作区 B");await page.getByRole("button",{name:"导入 / 导出"}).click();await page.getByRole("button",{name:"导入 CSV"}).click();
