...
test("维护属性并按位置创建子集后预览派生变化",async({page})=>{
  ...
  await page.route("**/api/workspaces/workspace-1/changes/preview",async route=>{
    ...
    return route.fulfill({json:{executable:true,requires_cad:true,changes:[{type:"number_range_changed",before:"001-002",after:"001-004"}],diagnostics:[],affected_files:["C:\\project\\test.dst","C:\\project\\003-004 新分册.dwg","C:\\project\\001-002 第一册.dwg"],execution_intent:{groups:[
      {operation:"create",subset_name:"003-004 新分册",target_file:"C:\\project\\003-004 新分册.dwg",layouts:[{number:"003",title:"新分册 (一)",layout_name:"003 新分册 (一)"},{number:"004",title:"新分册 (二)",layout_name:"004 新分册 (二)"}]},
      {operation:"rebuild",subset_name:"001-002 第一册",target_file:"C:\\project\\001-002 第一册.dwg",layouts:[{number:"001",title:"第一册 (一)",layout_name:"001 第一册 (一)"}]},
    ]}}});
  });
  ...
  await expect(page.getByText("图号范围变化")).toBeVisible();await expect(page.getByText("创建 DWG")).toBeVisible();await expect(page.getByText("重建 DWG")).toBeVisible();await expect(page.getByText("003-004 新分册",{exact:true})).toBeVisible();const createdGroup=page.locator(".execution-group").filter({hasText:"创建 DWG"});await expect(createdGroup.getByText("C:\\project\\003-004 新分册.dwg",{exact:true})).toBeVisible();await expect(createdGroup.getByText("003 新分册 (一)",{exact:true})).toBeVisible();
});

test("旧编辑入口已移除且图号标题只读",async({page})=>{
  await openWorkspace(page);await expect(page.getByRole("button",{name:"子集↑"})).toHaveCount(0);await expect(page.getByRole("button",{name:"子集↓"})).toHaveCount(0);await expect(page.getByText("移动到",{exact:true})).toHaveCount(0);await expect(page.getByRole("textbox",{name:"图纸图号"})).toHaveCount(0);await expect(page.getByRole("textbox",{name:"图纸标题"})).toHaveCount(0);await expect(page.getByText("001",{exact:true})).toBeVisible();await expect(page.getByText("第一册 (一)",{exact:true})).toBeVisible();
});
