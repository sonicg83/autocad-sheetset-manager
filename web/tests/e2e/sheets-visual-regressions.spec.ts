...
for (const theme of ["light", "dark"] as const) {
  test(`${theme} 主题下工具栏、属性和恢复按钮保持点击留白`, async ({page}) => {
    ...
    for (const selector of [".sheets-toolbar .operations button", ".recover-banner button", ".properties-view button", ".properties-view .definition-panel .link-actions button"]) {
