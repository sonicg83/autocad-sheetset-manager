...
<template><section class="panel repair" :class="`repair-${validation.status}`">
  ...
  <div v-if="validation.status==='REPAIRED'" class="link-actions"><UiButton variant="secondary" :disabled="previewing||executing" @click="$emit('previewRepair')">{{ $t("shell.repair.previewAndConfirm") }}</UiButton><template v-if="preview"><span class="derived">{{ $t("shell.repair.previewSummary",{count:preview.actions?.length??0,digest:preview.preview_digest?.slice(0,16)},preview.actions?.length??0) }}</span><UiButton variant="primary" :disabled="executing||!preview.executable" @click="$emit('executeRepair')">{{ $t("shell.repair.confirmPublish") }}</UiButton><UiButton variant="secondary" :disabled="executing" @click="$emit('cancel')">{{ $t("shell.repair.cancelConfirm") }}</UiButton></template></div>
</section></template>
