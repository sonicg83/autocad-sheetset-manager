...
function watchJob(id:string){
  ...
  events.onmessage=async event=>{job.value=JSON.parse(event.data);if(terminal(job.value!.status)){events.close();if(job.value!.status==="SUCCEEDED")await refreshWorkspace()}};
  events.onerror=()=>{events.close();connectionMode.value="轮询";setTimeout(()=>pollJob(id),1000)};
}
...
async function retryJob(){if(!job.value||!job.value.id)return;if(job.value.status==="NEEDS_REVIEW"){error.value="发布状态需要人工检查，禁止直接重试";return}const id=job.value.id;job.value=await request(`/api/jobs/${id}/retry`,{method:"POST"});watchJob(id)}
