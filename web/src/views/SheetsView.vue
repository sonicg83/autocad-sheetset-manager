...
const sheetEditContext = computed<PropertyEditContext | null>(() => props.editContext?.kind === "sheet" ? props.editContext : null);
      ...
      <main class="sheets-main">
        ...
        <template v-else>
          <SheetTable
            ...
            :all-filtered-selected="allFilteredSelected"
            :filtered-selected-count="filteredSelectedCount"
            :can-select="filteredRows.length > 0"
            :edit-context="sheetEditContext"
            ...
            @editor-set-value="(name, value) => $emit('editorSetValue', name, value)"
            @editor-set-page="(page) => $emit('editorSetPage', page)"
            @editor-set-search="(query) => $emit('editorSetSearch', query)"
            @editor-submit="$emit('editorSubmit')"
