...
- [图纸目录页面视觉收敛整改计划（PLAN-DM-023，proposed；依据用户真实桌面复验撤销 PLAN-DM-020 的 G8 通过结论，保持 G4 冻结件，收敛表格式编辑器、字段可见性、首屏预览/导出与响应式）](PLAN-DM-023-sheet-catalog-visual-convergence.md)
- [内置扩展平台与图纸目录 XLSX 实施计划（PLAN-DM-020，active；12 个实施任务已完成，但 G8 被 2026-09-11 用户真实桌面复验推翻；先执行 PLAN-DM-023 并重新通过 G8，再进行 G9）](PLAN-DM-020-sheet-catalog-builtin-extension.md)；G9 记录见 [MEMO-DM-028](../../memos/dst-manager/PLAN-DM-020-sheet-catalog-g9-checklist.md)。
