...
- [图纸标准属性与 DWG 命名整改实施计划（PLAN-DM-038，completed——Task 1–10 全部完成）](PLAN-DM-038-standard-properties-and-dwg-naming-remediation.md)：依据 SPEC-DM-017，以普通/派生属性和唯一全局 DWG 命名模板直接替换尚未发布的旧 Schema v1，并重构标准编辑与发布门禁；完成后 PLAN-DM-036 才能基于新契约实施创建流程（依赖关系已写入 PLAN-DM-036）。实际验证：ruff、pytest **1697 passed / 72 skipped / 0 failed**、uv lock、check:api/check:i18n（1292 键）/check:ui/build 全过、test:unit 260、全量 e2e **619 passed / 0 failed**；残余风险见计划 §实际验证摘要。
