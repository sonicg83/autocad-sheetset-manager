...
- [图纸标准平台欢迎页与编辑器视觉收口实施计划（PLAN-DM-039，active——Task 1–4 自动环节已完成，**仅剩 Step 6 真实 WebView2 缩放检查待用户执行**）](PLAN-DM-039-standard-platform-ui-visual-closure.md)：承接 PLAN-DM-035/038 已接通功能但遗漏的页面级布局与视觉证据——重建“打开 DST 优先”的欢迎页约 2:1 双栏，把标准草稿切换为独立全宽六分区编辑工作台，并以候选截图、用户 Demo 三轮人工裁决、新 G4/G8 和真实 WebView2 100/125/150/200% 检查作为关闭条件；不修改标准 Schema、后端 API 或 PLAN-DM-036 创建流程。实际验证：欢迎页双栏+唯一 primary+三个带说明的次级入口、编辑器互斥页面模式与分级响应式（238px → 210px → 单列）、属性表回归 Demo 单行密度与“摘要即触发器”枚举入口；`test:unit` **275 passed**、`check:api`/`check:i18n`（1305 键）/`check:ui`/`build` 退出码 0、全量 e2e **630 passed / 0 failed / 0 flaky**、`ruff` 与 `uv lock --check` 通过、`pytest` **1628 passed / 72 skipped / 0 failed**（1700 项）；G4/G8 各 12 张重建且逐对 SHA-256 **12/12 一致**，旧四个状态文件已删除；用户已三轮裁决并**通过**，证据与裁决见 [PLAN-DM-039 资产登记](../../../memos/dst-manager/assets/PLAN-DM-039/README.md)。
