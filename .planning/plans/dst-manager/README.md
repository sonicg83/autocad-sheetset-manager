...
- [标准包导入受控文件选择修复计划（PLAN-DM-041，proposed）](PLAN-DM-041-standard-package-import-picker.md)：独立承接原 F16／PLAN-DM-039 F7，补齐 `.dststandard` 原生选择、无壳回退、导入弹窗焦点与 G9 验收。
- [图纸标准平台审查问题修复计划（PLAN-DM-040，proposed）](PLAN-DM-040-standard-platform-review-remediation.md)：承接 PLAN-DM-035/038/039 审查和草稿编辑实测的 F01–F15 与 F17（身份路由越界）共 16 项问题；本机模板文件无法纳入草稿列为阻断级，先修资产与路径安全，再修编辑状态和 UI 契约；10 个任务按 Step 展开并要求记录 RED/GREEN，原 F16 拆至 PLAN-DM-041。
- [图纸标准平台欢迎页与编辑器视觉收口实施计划（PLAN-DM-039，completed——Task 1–4 全部完成）](PLAN-DM-039-standard-platform-ui-visual-closure.md)：承接 PLAN-DM-035/038 已接通功能但遗漏的页面级布局与视觉证据——重建“打开 DST 优先”的欢迎页约 2:1 双栏，把标准草稿切换为独立全宽六分区编辑工作台，并以候选截图、用户 Demo 三轮人工裁决、新 G4/G8 和真实 WebView2 100/125/150/200% 检查作为关闭条件；不修改标准 Schema、后端 API 或 PLAN-DM-036 创建流程。实际验证：欢迎页双栏+唯一 primary+三个带说明的次级入口、编辑器互斥页面模式与分级响应式（238px → 210px → 单列）、属性表回归 Demo 单行密度与“摘要即触发器”枚举入口；`test:unit` **275 passed**、`check:api`/`check:i18n`（1305 键）/`check:ui`/`build` 退出码 0、全量 e2e **630 passed / 0 failed / 0 flaky**（4.3 分钟）、`ruff` 与 `uv lock --check` 通过、`pytest` **1628 passed / 72 skipped / 0 failed**（1700 项）；G4/G8 各 12 张重建且逐对 SHA-256 **12/12 一致**，旧四个状态文件已删除；用户已三轮裁决并**通过**；真实 Windows WebView2 浅/深 × 100/125/150/200% 检查覆盖欢迎页/普通属性/DWG 命名/组合模态/发布检查并**全部通过**（无裁切、无页面级横向滚动、操作栏可见、键盘可达）。证据与裁决见 [PLAN-DM-039 资产登记](../../../memos/dst-manager/assets/PLAN-DM-039/README.md)。
- [图纸标准属性与 DWG 命名整改实施计划（PLAN-DM-038，completed——Task 1–10 全部完成）](PLAN-DM-038-standard-properties-and-dwg-naming-remediation.md)：依据 SPEC-DM-017，以普通/派生属性和唯一全局 DWG 命名模板直接替换尚未发布的旧 Schema v1，并重构标准编辑与发布门禁；完成后 PLAN-DM-036 才能基于新契约实施创建流程（依赖关系已写入 PLAN-DM-036）。实际验证：ruff、pytest **1700 passed / 72 skipped / 0 failed**、uv lock、check:api/check:i18n（1293 键）/check:ui/build 全过、test:unit 266、全量 e2e **620 passed / 0 failed**；残余风险见计划 §实际验证摘要。
...
- [标准驱动的新图纸集创建实施计划（PLAN-DM-036，proposed；已按 SPEC-DM-018 四阶段 UI 修订）](PLAN-DM-036-standard-driven-sheetset-creation.md)：从已发布标准按图纸组生成 DST/DWG，支持双表 XLSX、内置受控 DST 骨架、按组预览与可恢复发布；经真实 CAD 验证后直接进入普通工作区。
