...
Task 1–10 提交后，对 `6231d19..HEAD` 全分支 diff 做了一次独立只读复核（Review Focus 逐条 + 安全边界 +
契约兼容 + 前端状态机 + 提交范围）。复核结论为「OK with notes」，并给出三项高优先级问题，已同批修复并验证：

1. 窄屏（≤959px）在详情页删除当前草稿后停在「列表隐藏 + 详情空态（无返回入口）」的死路：删除成功后复位两级视图。
2. 两个资产声明同一路径时导出会写出重复 ZIP 条目，自家阅读器会拒绝自家导出包：导出按包内路径去重，回归断言「导出后必须能再次导入」。
3. 标准库根下混入非法目录名（如版本段 `tmp`）时创建候选列表整表 422：发布目录扫描改为与草稿扫描同口径跳过非法名。

同批修正的次级问题：检查本身失败时布局表仍给出「缺少同名布局」的假结论；草稿态详情错误的「重试加载详情」
按钮无动作；200% 缩放用例的返回按钮视口断言原为恒真（改用页面内 `window.innerHeight`）。

修复波后重跑全量门禁：Ruff、pytest、vitest（314 例）、`build`、Playwright（678 passed）全部通过。
