...
- [x] **Step 1: 先更新 E2E 夹具为新 Schema 并写失败流程**
...
**交付范围**：Task 1–10 全部完成，分支 `feature/plan-dm-038-standard-properties-and-dwg-naming`（自 `main` caa1ead）。

- **领域层**：Schema v1 直接替换——`standard_models.py`（冻结模型 + 反向引用）、`standard_schema.py`（结构解析）、`standard_semantics.py`（发布语义门禁）、`standard_rules.py`（映射/组合求值与发布诊断）、`standard_naming.py`（唯一 DWG 命名模板与文件名安全）、`standards.py`（两阶段入口门面）。旧 `StandardRule`/顶层 `rules`/自由规则目标已删除，不保留兼容层。
