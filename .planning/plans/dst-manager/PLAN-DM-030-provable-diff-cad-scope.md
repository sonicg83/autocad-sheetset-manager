...
- **2026-09-14 追记：** 本计划验证中发现第二个缺陷（删除已应用的不编号封面使后续子集重编为 `00 图纸目录` 并按改名进入 CAD），根因在编号派生而非 CAD 工作范围，已由 [PLAN-DM-032](PLAN-DM-032-delete-unnumbered-subset-number-seed.md) 修复；两项修复互为验证：删除不编号子集既不重编号、也不产生 CAD 工作单元。本计划的结论（前沿只作展示）不变。
