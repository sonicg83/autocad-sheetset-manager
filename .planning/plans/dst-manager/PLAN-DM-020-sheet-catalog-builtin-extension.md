...
2026-09-11：用户以真实 Windows 桌面首屏与完整页面截图复验后，明确推翻 2026-09-10 的 G8“通过”结论：除输出列 `↑`、`↓`、`✕` 图标操作按钮外，卡片式输出列、字段搜索与常显语法缺失、独立兼容/操作卡、首屏无法看到预览与导出、可用态冗余状态区及 900px 布局均未获接受。现行裁决见 [MEMO-DM-030](../../memos/dst-manager/2026-09-11-plan-dm020-g8-user-revalidation.md)；SPEC-DM-012 G8 已恢复为“未通过”，G9 暂停。G4 commit `9f3dfb3` 继续作为冻结设计，不因生产偏离而重开；已立项 [PLAN-DM-023](PLAN-DM-023-sheet-catalog-visual-convergence.md) 将生产实现收敛回冻结件。PLAN-DM-020 继续保持 `active`，待 PLAN-DM-023 完成、用户重新通过 G8 后再执行 G9。
...
2026-09-11：[PLAN-DM-023](PLAN-DM-023-sheet-catalog-visual-convergence.md) 的 6 个任务已全部实施完毕（生产改动 commit `441b85c`（红灯）`1cf0a8f`/`0d69e45`/`70b23c0`/`241c7b5`）：输出列恢复表格式、字段搜索与两行条目恢复、兼容性摘要嵌回输出列卡、刷新/导出入预览上下文、可用态头部与模板栏压缩、四档视口响应式收敛。本轮新鲜验证（全部退出码 0）：`uv sync --dev`、`uv run ruff check .`、`uv run pytest`（1121 passed / 72 skipped / 0 failed）、`uv lock --check`、`check:api`、`check:i18n`（894 键 / 9 域）、`npm run build`、全量 E2E 432 passed / 0 failed / 1 flaky（同前 dev server 抖动，单独 `--workers=1 --retries=0` 复现 1 passed）。生产证据已从被忽略的 `.superpowers/` 改为入版本库：`docs/dst-manager/specs/assets/SPEC-DM-012/production/g8-catalog-light-1440x1000.png`（1440×1000）与 `g8-catalog-dark-900x700.png`（900×700）。逐对比对（V1～V8、A1/A2）与三项待裁决差异候选见 [MEMO-DM-030 §6](../../memos/dst-manager/2026-09-11-plan-dm020-g8-user-revalidation.md)。SPEC-DM-012 §16 的 G8 现记为“待用户复核（PLAN-DM-023 已实施）”；**实施代理未自行登记 G8 通过、未自行接受任何差异**；本计划继续保持 `active`，G9 继续暂停，待用户逐对确认后收口。
