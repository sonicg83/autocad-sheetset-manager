...
**目标：** 把图纸目录专用设置分派升级为受信 Builtin 扩展通用的当前 Windows 用户级设置框架，提供 Provider 语义、generated/custom 两类设置入口、Schema 迁移、乐观并发和 preview/execute 设置快照门禁。
...
| `src/dst_manager/extensions/builtin/sheet_catalog/settings.py` | 图纸目录 Provider，复用模板纯业务函数 |
...
- [ ] **步骤 2：运行红灯。**
  ...
  ```powershell
  rtk uv run pytest tests/unit/test_extension_settings_service.py tests/unit/test_sheet_catalog_templates.py tests/integration/test_extension_api.py -q
  ```
...
- [ ] **步骤 7：运行绿灯。**
  ...
  ```powershell
  rtk uv run pytest tests/unit/test_extension_settings_service.py tests/unit/test_extension_persistence.py tests/unit/test_sheet_catalog_templates.py tests/integration/test_extension_api.py -q
  ```
...
- [ ] **步骤 2：写摘要/执行漂移红灯。** 只改设置身份也改变摘要；预览后 PUT 再执行返回 409、授权未消费、没有 Artifact；预览结束后的 best-effort“上次选中模板”偏好写入不得使该预览自行过期。
  ...
  put_settings(client, changed_templates(), expected_revision=preview["settings_revision"])
...
**设计裁决：** 卡片本体继续不可点击；仅声明设置时显示文字按钮“配置”。配置进入同一 Settings `<dialog>` 子视图，使用可见“返回扩展列表”，不叠加设置模态。每个扩展独立保存；子视图 dirty 纳入返回、Esc、遮罩和关闭确认。

- [ ] **步骤 1：更新 SPEC-DM-011。** 修订 SC-16，并新增 SC-17：统一配置入口、generated/custom、无工作区访问、独立保存、焦点归还和脏状态闸门。

- [ ] **步骤 2：更新 Demo。** 增加有/无设置卡片、generated 表单、custom 模板设置、冲突和只读状态；动作行 DOM 顺序固定为“配置按钮 → 状态文字 → 开关”，可访问名为“配置 {name}”。
...
- 新建：`web/src/composables/useSheetCatalogTemplates.ts`
- 修改：`web/src/composables/useSheetCatalog.ts`
...
- [ ] **步骤 2：更新 GUIDE-DM-005。** 替换专用设置 `if` 说明，加入 Provider 模板、两类呈现选择表、迁移/高版本/快照测试和新增扩展 SOP。
...
- 业务页和设置页不能各持一套模板语义；先抽取 `useSheetCatalogTemplates`，并行编辑靠 revision 冲突。
