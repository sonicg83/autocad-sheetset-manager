...
**T9-2（续轮途中：Step 3 完成 + Step 5 落点裁定 + 裁定差异登记）**

- **Step 3 已完成**（提交 `66cafc2`）：`ConfirmModal.vue` 与 `UnsavedInputDialog.vue` **均已接入 `useDialogFocus`** ✓；**公开 props/emits/插槽契约未变**（diff 不碰 `defineProps`/`defineEmits`）✓；SPEC-DM-006 的原生 dialog 裁决保留 ✓。它额外搞清并**写进代码注释**两个顺序事实：① 闸门需 `showModal()`/`close()` 在焦点工具**之前**执行；② `opener` 必须在 `showModal()` **之前**捕获（原生模态会把焦点移入）。`initialFocus: card` 保持原落点。“差异”均为**更严的过滤/更少的副作用**，非语义变更。
- **变异自证 3 例（含一例诚实的结果）**：A（欢迎页 38px）→ 红 `Expected: 38 / Received: 34` ✓ 活；B（移除 keydown 绑定）→ 红 ✓ 活；**C（闸门 `returnFocus` → null）→ 仍绿** → 故该接线是**防御性安全网而非常规载荷**（原生 `<dialog>` 自己会归还焦点）。**它选择如实报告而非声称已证** ✓——这比“全部通过”更有价值：一个**不会变红**的变异恰恰证明该守卫不承担回归网职责。
- **Step 5 落点裁定（worker 正确地没有自创路径）**：它核实并报告了三个事实（控制器已逐条复核为真）——`assets/` 下只有 SPEC-DM-009/011/012/013（**无旧页面资产目录**）；Task 9 的 Files **没有**任何 `*visual-evidence*` spec；`main.spec.ts` **零截图**。
  - **裁定**：① **新建 `docs/dst-manager/specs/assets/SPEC-DM-006/production/`** 作为落点（SPEC-DM-006 是桌面 UI/UX 的**总纲 Spec**，正是旧页面的归属；先例：T7-4 新建 SPEC-DM-009 的资产目录）；② **接纳其提议：证据组写在 `main.spec.ts`**（该文件**在 Files 内**）——理由：4 个状态的夹具流程**已在其中**（修复 `:608`、深色 `:769+`、修订流程、欢迎页），另建 `*-visual-evidence.spec.ts` 会**重复不易写的夹具逻辑** ✗；且 `main.spec.ts` 本就是旧页面/壳层的行为 spec。**此处对 `<scope>-visual-evidence` 命名惯例的偏离是刻意的**，记录在此以免后人误认为疏漏。
...
- [x] **Step 8（计划关闭）**：全量自动门禁已全绿；真实桌面复验（Step 5–6）未由用户执行，**计划归属方（用户）于 2026-09-16 明确裁定以当前状态关闭**（该裁定记录于本节末尾与 changelog），状态改为 `completed`，两个索引与 changelog 已同步。
- [x] **Step 9（最终提交）**：2026-09-16 归档提交 `完成前端视觉基础整改验收闭环` 并经用户确认合并 main。
...
**T12-2（第 1 轮完成 + Step 1/2/3 控制器亲跑 + 责任 A–Y 收口表 + Step 8 状态判定）**

**第 1 轮产出（worker，5 个提交）**：`42472fe` C②加固 · `c6b6b38` 字体真实加载 + 壳层布局断言 · `0195397` SPEC-DM-010 的 36px 豁免 · `f4e1e92` ARCH-DM-007 令牌分层约束 · `1ef20db` changelog。
**它自查出四个自身错误（均被机制抓住而非顺过）**：① 探针把 `URL.host`（**含端口**）当远程判定 → e2e 失败暴露；② 据启发式文本误判默认页签 → **被浏览器实测推翻并显式报出**；③ **4 条变异同跑互相掩盖**（①在②下游）→ 一度误判「② 未覆盖」，逐条隔离后纠正 ✓✓；④ 编辑时误删 `return violation({` 一行 → 立即复核发现并修复（未进提交）。
**两处对控制器/计划原文的修正（均有证据）**：**责任 E 的定位**——规则实际在 **§4.1（:113）**而**非 §3**（计划与 T12-1 均写错）；**实测值 1025 处 / 44 文件**，**非旧的 1104** ✓✓。

**Step 1/2/3（控制器亲跑，真实 EXIT）**：`test:contracts` **0（86/86）** · `test:unit` **0（13 文件 / 166 passed）** · `build` **0**（含 `check:api`/`check:i18n`/`check:ui`/`vue-tsc -b`/`vite build`） · **全量 e2e 矩阵 0（548 passed / 2 flaky / 3.6m）** · `ruff` **0** · `uv lock --check` **0** · `pytest` **0（tests=1488 / failures=0 / errors=0 / skipped=74 / 89.6s，与隔离基线逐值一致）**。
- **2 条 flaky 均为同一签名**：`locator.click: Test timeout of 30000ms exceeded` 等待**外壳引导按钮**（`选择 DST 文件`）⇒ 即已登记的**引导期 flaky** ✓（非行为差异）；**不得用放宽断言/提高超时来“修”** ✓。
- **pytest 的警告均为既有且与本计划无关**（Starlette/httpx、Python 3.12 sqlite3 datetime、pytest ini `path_separator`）✓。
- **工具陷阱（新，同族于「`tail` 的退出码」）**：**harness 会截断重定向输出**（`pytest -q > file` 只得 57 行、停在 14%）且包装器会打出**假的** `Pytest: No tests collected` ✗ → **需要计数时改用 `--junit-xml` 并解析 XML** ✓。

### 责任 A–Y 收口表（逐项定性）

| 责任 | 状态 | 依据 / 归属 |
|---|---|---|
| **A** 字体真实加载未覆盖 | ✅ **闭合** | 本轮补运行时断言 + 负控 + `url(/assets/…)` 产物核验 |
| **B** 字体子集化命令不可复原 | ⚠️ **环境阻碍** | 需可访问上游的环境重做；**禁止**用推测命令文本或其他环境复原物替换已入库资产 |
| **C①** `rule` 字段冗余 | ⏸ 已按设计推迟 | 原文本即写「下次重新生成例外表时」 |
| **C②** 例外表可自掩蔽自身配置错 | ✅ **闭合** | 本轮加固（**RED 先证明漏洞真实可利用**）+ 单测 |
| **C③** 注释–指纹耦合 | ✅ **闭合** | **实测：当前 14 条中内嵌前置注释者 = 0** ✓（其宿主已在前几轮清理）|
| **E** 令牌分层未落实 | ✅ **闭合** | 约束写回 **§4.1**（非 §3）+ 实测 **1025/44**（非旧 1104）；**未新造色板** ✓ |
| **F** `fieldset[disabled]` 继承未建模 | ⏸ 待裁决 | 本仓当前**不可达**；含 F③（`SettingsDialog` 同型）|
...
| **O** SPEC §232 文本域无原语 | ⏸ 待裁决 | 另含 Task 10 的「密集单行控件」原语缺口 |
| **P** 视觉证据注释与实现不符 | ✅ **已闭合** | Task 5 第二轮修复已订正（保留记录）|
| **Q** 输入 hover vs §5.1「+4%」 | ⏸ **交 Spec 归属方** | 已成原语契约并被 Task 6–9 沿用；**定调前不得声称符合 §5.1** |
...
### Step 8 状态判定：~~保持 `active`~~ → **`completed`**（2026-09-16 用户裁定关闭）

> **关闭裁定（2026-09-16）**：Steps 5–6（真实 Windows WebView2 100/125/150/200% 复验）**未执行**，证据缺口如实保留于责任 A–Y 收口表。原判定条件「真实桌面复验通过才可关闭」未被满足，但计划归属方（用户）在审阅审查修复轮与用户验收修复轮的门禁实绩（契约 96/96、单测 168、全量 e2e 554 passed/1 flaky、build/ruff/pytest 全绿）后**明确裁定以当前状态关闭本计划**。真实桌面缩放复验仍建议在后续版本迭代中补做，其风险面（缩放裁切、焦点丢失、点击面积）已由 e2e 几何断言与视觉证据部分覆盖，不能视为替代。

原判定理由（真实桌面复验未完成故保持 `active`）保留如下，作为历史记录：
...
**T12-4（责任 K 的**另一半**：14/18px 语义档位 + 抹掉 5 处跨层借用）**

**★ worker 自己的收窄——这一条必须记下**：T12-3 只闭合了责任 K 的「**离刻度值**」那一半；它在 ARCH-DM-007（`:121`）与 `tokens.css` 头注释里**如实记为「仍然开放的另一半」**，并把自己的结论由「责任 K 就此闭合」**收紧为「离刻度值这一半就此闭合」** ✓✓——**主动缩小自己成果的适用范围，而不是借一次裁定把整条债清零**，属本计划一贯标准的最佳示例。

**用户裁定（2026-09-15）的原话是「新增语义字号档位」** ✓（未限定仅那 4 个值）⇒ **同一原则直接适用于 K 的原始主题**（「语义层缺独立 `14px`/`18px` 档位」） ⇒ 本轮把它做完 ✓：
1. **新增 2 个语义字号档位**（值逐字等值：**14px** 与 **18px**），放进与 `--font-panel-title` 等**同一块**；命名按**角色**、不得含页面名（由实现者拟定并在报告列出）。
2. **把 5 处跨层借用改为消费新语义令牌**：`SheetCatalogView.vue:154`（借 `--modal-title-font-size`）· `CatalogPreview.vue:55` · `TemplateBar.vue:145` · `FieldBrowser.vue:209` · `ColumnEditor.vue:200`（借 `--button-font-size`）。**只改字号声明** ✓。
3. **零视觉变化**（值等值）+ 沿用/补绝对值锚 + **变异自证** ✓。
4. **例外表不受影响**（借用不属例外）：应仍为 **7** / 裸违规 **8**；**不得新增例外** ✓。
