...
## 真实桌面复验：逐项检查清单与记录模板（控制器提供，对应计划 Task 12 Step 5–6）

> **前提（计划 Step 6 原文）**：**真实桌面缩放证据不能由浏览器 zoom 代替**。请在**真实 pywebview/WebView2 外壳**里操作，**不要**用浏览器窗口缩放或 Playwright 的 `deviceScaleFactor` 顶替——**伪造的证据比没有证据更坏**。
> 使用**虚构/样本**工作区；**不要**在记录里写真实客户路径或其它敏感数据。

### 一、缩放比例（Windows「设置 → 显示 → 缩放」）

对 **100% / 125% / 150% / 200%** 各做一轮；每轮至少覆盖：

- **壳层**：顶栏（含 CAD 版本选择、关闭、设置）、页签栏、操作坞、任务浮层（实施进度/修改预览/诊断三页签）
- **一个密集页面**（属性页 / 图纸目录页 / 图纸页 任选其一，但四个比例之间请保持一致以便横向对比）

### 二、125% **必须**覆盖的三个缺陷场景（对应用户三张截图）

| 用户截图 | 场景 | 对照的入库证据（同状态「修复后」） |
|---|---|---|
| 第 1 张 | **图纸页**（Task 7） | `docs/dst-manager/specs/assets/SPEC-DM-009/production/task7-*.png`（7 张） |
| 第 2 张 | **图纸目录页**（Task 6） | `docs/dst-manager/specs/assets/SPEC-DM-012/production/t6-*.png`（5 张） |
| 第 3 张 | **属性页**（Task 5） | `.planning/memos/dst-manager/assets/PLAN-DM-029/properties-*.png`（5 张） |

> 说明：用户三张原图**未入库**，故上表是**页面级**映射；若你在原图上看到上表未覆盖的具体项，请直接按该项检查并记录。

### 三、每个「比例 × 页面」要看的五类问题

字体突变（字号/字族与 100% 不一致）· 文本或控件被裁切 · 出现横向溢出/整页横滚 · 焦点丢失（弹窗/浮层打开后焦点不在预期位置）· 点击面积不足（可点元素明显小于 32px 或难以点中）。

### 四、记录模板（可逐行复制使用）

| 比例 | 页面 | 字体突变 | 裁切 | 溢出 | 焦点丢失 | 点击面积不足 | 备注 / 截图文件名 |
|---|---|---|---|---|---|---|---|
| 100% | 壳层 | | | | | | |
| 100% | 密集页 | | | | | | |
| 125% | 图纸页 | | | | | | |
...
- **P0/P1/P2 必须清零**；**P3** 只有在不影响任务完成与信息层级时才可登记为后续项。
- **自动截图存在不代表通过**；请把上面的表**逐格填满**再下结论。

### 六、结果回填后会发生什么（控制器动作）

- **全部通过** ⇒ 计划状态改为 `completed`，控制器同步两个索引与 `changelog.md`。
- **发现 P0/P1/P2** ⇒ 控制器开修复轮（先定位修复，**不放宽断言、不改超时**）。
- **未做** ⇒ 计划**保持 `active`**，并在索引/changelog 里如实写明「仅差真实桌面一项」。
