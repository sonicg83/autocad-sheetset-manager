...
# 扩展平台设置码（PLAN-DM-025 Task 3/4；GET 只读视图与 PUT 409 共用同一稳定 code，
# 预览后设置变化由动作执行返回同一目录的稳定 code）
EXTENSION_CODES = {
...
CREATION_CODES = {
    ...
    "CREATION_ASSET_PATH_INVALID",
    "CREATION_CAD_FAILED",
    "CREATION_CAD_UNAVAILABLE",
    "CREATION_DRAFT_CONFLICT",
    "CREATION_DRAFT_CORRUPT",
    "CREATION_DRAFT_INVALID",
    "CREATION_DRAFT_NOT_FOUND",
    "CREATION_DST_INVALID",
    "CREATION_HANDLE_DUPLICATE",
    "CREATION_HANDLE_INVALID",
    "CREATION_HANDLE_PLACEHOLDER",
    "CREATION_HANDLE_SET_MISMATCH",
    "CREATION_IMPORT_INVALID",
    "CREATION_JOB_ID_INVALID",
    "CREATION_LAYOUT_READ_FAILED",
    "CREATION_LAYOUT_REFERENCE_INVALID",
...
# 标准库发布与导入（PLAN-DM-041 Task 3；服务端分配整数版本、名称唯一与发布写入）
STANDARD_CODES = {
...
def test_settings_changed_code_reuses_extension_message_key():
    """PLAN-DM-025 Task 4：预览后设置变化（409）的稳定码必须进 UI 可见目录，
    否则 extension_api._error_response 按码索引文案键会直接 KeyError → HTTP 500。"""
    from dst_manager.interfaces.extension_contracts import EXTENSION_MESSAGE_KEYS

    assert "EXTENSION_SETTINGS_CHANGED" in known_codes()
    assert CATALOG["EXTENSION_SETTINGS_CHANGED"].message_key == (
