...
- 现役 scope 只使用 `dst-manager`、`shared` 和 `integration`：`dst-manager` 是唯一现役产品；`shared` 只接收 Manager 已采用的稳定公共能力；尚在讨论的提案、契约和合并议题进入 `integration`，不得因“未来可能复用”提前归入 `shared`。`dst-builder` 与 `legacy-refactor` 都是历史 scope，只保留历史资料，不接收新的产品需求、架构、Spec、计划或待办。
...
- `.planning/plans/`、`.planning/todos/` 和 `.planning/memos/` 下按 scope 分目录：现役目录只使用 `dst-manager`、`shared` 和 `integration`；`dst-builder` 等历史目录只读保留，不新增条目；Roadmap 使用 scope 对应的单文件入口。
- 新建文档前先判断生命周期、scope 和文档类型，并搜索同主题权威文档；同一内容只保留一个权威位置，其他位置使用链接，不复制正文。不要将待办或备忘混入 `docs/`，也不要将长期知识文档放入 `.planning/`。
- PRD、Spec、Architecture、ADR、RFC、Roadmap 和 Plan 必须使用 YAML 元数据；新正式文档使用永久 ID（类型 + `DM`/`SH`/`INT` + 三位序号），编号不得重用。历史 `DB` 与 `LR` 编号永久保留，但只对应历史 scope，不再新增。`related` 使用文档 ID，正文仍提供可点击链接。
