...
3. **序号种子与图号位数**：非不编号子集的首个纯数字图号决定序号起点与位数（既有规则）。计算种子时**跳过**不编号子集，避免其中必然出现的 `000` 把起点拉成 0。跳过范围必须与种子取值的快照一致：种子读的是**命令前**文档的既有图号，因此排除集合必须按**命令前**标题判定（本批命令中被删除或改名的首位不编号子集也必须排除），再与命令后子集列表的判定取**并集**。若文档内不存在任何非不编号子集的数字图号，则借用文档既有数字图号的位数（起点仍为 1）；文档内确实没有数字图号时回退为 1 位。
4. **图号取值**：不编号子集内的每张图纸图号恒为 `0` 重复到项目位数（如 3 位项目为 `000`）。这是**单一取值，不是编号范围**：子集内无论多少张图纸，子集显示名与图号范围都显示 `000`。
5. **不消耗序号**：遍历不编号子集时不递增序号计数器。因此在该子集中新建图纸、删除图纸、或新建/删除一个命中关键字的子集，**不改变任何其他子集的图号**；编号效果与「该子集不存在于编号序列中」一致。不编号子集排在中间时，其后的施工图子集仍从上一个非不编号子集继续编号。
...
**实现缺陷修复（2026-09-14 追记）：** 用户实测发现**删除**已应用的不编号封面后，紧随其后的子集被重编为 0 起（如 `01 图纸目录` → `00 图纸目录`）；若封面尚未应用 0 填充（关键字刚开启），删除它则使后续子集整体前移。根因在编号派生实现：`_number_seed` 读的是**命令前**文档的既有图号，而「不编号子集」排除集合按**命令后**子集列表判定，被本批命令删除的封面已不在集合中，其 `000`/`00` 因此被当成编号种子（§行为 3 的「跳过不编号子集」在删除路径上失效）。修复：命令前快照的判定与命令后判定取并集后再求种子，`§行为 3`、`§行为 5` 已按此收窄表述，领域规则本身不变。回归证据见 [PLAN-DM-032](../../../.planning/plans/dst-manager/PLAN-DM-032-delete-unnumbered-subset-number-seed.md)（先红后绿：`tests/unit/test_unnumbered_subsets.py` 新增 4 例、`tests/unit/test_core.py` 新增 1 例计划级用例，全量 1481 项 / 1409 passed / 72 skipped / 0 failed）。
