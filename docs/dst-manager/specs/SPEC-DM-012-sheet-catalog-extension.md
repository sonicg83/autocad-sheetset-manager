...
1. 模板栏：当前模板、兼容状态、保存、另存为、删除。模板修改状态与保存动作遵循 [SPEC-DM-015](SPEC-DM-015-frontend-text-edit-state-contract.md)：采用模板级中性/警示徽标，clean 时“保存修改”保持可聚焦但语义禁用；本轮不增加单元格级或行级 dirty 提示。
...
| 模板 | 内置、用户模板、未命名草稿、已保存、已修改（模板级警示徽标）、改回原值、保存中、保存失败、删除确认；clean 时保存修改不得发请求 |
