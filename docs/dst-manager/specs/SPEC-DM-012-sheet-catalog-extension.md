---
id: SPEC-DM-012
title: 图纸目录 XLSX 内置扩展设计规范
status: accepted
document_kind: spec
owners:
  - dst-manager
created: 2026-09-09
updated: 2026-09-12
related:
  - PRD-DM-001
  - ARCH-DM-006
  - ARCH-DM-005
  - SPEC-DM-006
  - SPEC-DM-009
  - SPEC-DM-010
  - GUIDE-DM-001
  - PLAN-DM-020
  - PLAN-DM-023
  - PLAN-DM-024
  - PLAN-DM-025
  - PLAN-DM-026
  - RES-DM-001
  - MEMO-DM-030
  - MEMO-DM-031
---

# 图纸目录 XLSX 内置扩展设计规范

> 定位：首个内置业务扩展 `dst-manager.sheet-catalog` 的权威行为与交互 Spec。扩展加载、Capability、保存授权和 Artifact 边界以 [ARCH-DM-006](../architecture/ARCH-DM-006-builtin-extension-platform.md) 为准；前端门禁以 [GUIDE-DM-001](../guides/GUIDE-DM-001-frontend-design-implementation-gates.md) 为准；G4 评审使用[图纸目录交互 Demo](../mockups/SPEC-DM-012-sheet-catalog-demo.html)。

## 1. 背景与范围

用户需要把当前 DST 中的图纸信息导出为 Excel 可直接使用的图纸目录。首版不应只是固定下载端点：后续需要按不同项目选择图纸集属性、图纸属性和组合规则，并保存可跨图纸集复用的模板。

本 Spec 覆盖：

- 独立“图纸目录”工作区页面；
- 内置默认模板和应用级用户命名模板；
- 固有字段、图纸集自定义属性、图纸自定义属性及组合表达式；
- 字段引用尾部的数字补零/去零格式码（§5.4），用于统一图号宽度而不修改 DST 数据；
- 当前 Windows 用户级“输出图纸过滤”设置，按图名关键词排除目录行；
- 当前图纸集字段兼容性、空值警告和真实数据预览；
- 用户选择保存位置、XLSX 生成和成功 Artifact 登记；
- 浅深主题、键盘、视口、失败恢复和桌面壳验收。

不包含：

- CSV、PDF、多工作表、公式、图表、宏或 Excel 模板文件导入；
- 条件表达式、函数、循环、数学运算、任意格式化管道或脚本（仅 §5.4 定义的封闭数字格式码除外）；
- 自动增加缺失 DST 属性、修改属性值或产生 `Change Proposal`；
- 除 §6.4 全局图名关键词排除外的图纸筛选、排序、分组、小计或跨多个 DST 合并；
- 通用 Artifact 管理页面和后台长任务；
- 浏览器开发态手工输入任意输出路径。

本项目为 L 级跨域变更，后端平台骨架与本独立页面作为同一纵向试点，但必须按批次独立验证。

## 2. 用户目标与成功标准

### 2.1 主要任务

1. 进入独立“图纸目录”页面并选择一个已保存模板。
2. 查看当前图纸集可用字段及模板兼容状态。
3. 用“输出列名 + 表达式”配置目录列并即时预览。
4. 将有效配置另存为可跨图纸集复用的命名模板。
5. 选择最终保存位置并导出可由 Excel 打开的 XLSX。

### 2.2 可观察成功标准

- 默认模板无需配置即可导出“图号、图名、文件名”三列。
- 用户能够生成 `{sheet.专业代码}-{sheet.number}`，结果如 `RQ-002`。
- 用户能够用 `{sheet.number:0000}` 把图号 `01` 输出为 `0001`、用 `{sheet.number:0}` 输出为 `1`；空值和非纯数字值原样输出，不被伪造、截断或补零。
- 用户能够在未打开工作区时配置“输出图纸过滤”；图名包含任一关键词的图纸不进入预览或 XLSX，预览只汇总过滤数量。
- 模板引用当前图纸集未定义的属性时，页面明确指出作用域和属性名，并禁止导出。
- 属性已定义但部分记录为空时，页面显示数量警告并允许导出，空值不被伪造。
- 用户能在原生“另存为”中选择目标位置；成功后可直接找到并打开文件。
- 页面不默认展示 Artifact、来源修订和哈希等技术信息，但后台记录完整可追溯。
- 预览和导出不修改 DST/DWG，不创建工程内 `.dst-manager/`。

## 3. 用户流程

### 3.1 主流程

1. 工作区加载且扩展可用时，宿主显示独立“图纸目录”页面。
2. 页面加载内置默认模板、用户模板、上次选择、当前 Windows 用户级过滤设置和当前工作区字段目录。
3. 用户选择模板；宿主立即对当前只读快照执行语法和字段兼容性校验。
4. 用户可修改输出列名、表达式和顺序；修改形成页面本地未保存草稿。
5. 页面防抖请求预览，显示固定数量的真实结果行、总行数、错误和警告。
6. 用户可“保存修改”或“另存为模板”；保存模板不是导出的前置条件。
7. 用户点击“导出 XLSX”；只有当前预览有效且与草稿一致时继续。
8. 桌面壳打开原生“另存为”；用户取消则回到页面，草稿和预览保留。
9. 用户确认路径后，宿主执行修订复核、生成、校验、原子保存和 Artifact 登记。
10. 成功时仅显示“图纸目录已保存到……”，并提供“打开所在文件夹”；失败时保留草稿和预览以便重试。

### 3.2 模板切换与离开

- 无未保存修改时可直接切换模板。
- 有未保存修改时，切换模板、离开页面、停用扩展或关闭工作区必须提供“保存为模板、放弃修改、留在此处”选择。
- 内置默认模板不可直接修改；首次编辑即成为未命名草稿，只能“另存为模板”。
- 用户模板允许原位保存；模板名大小写不敏感地唯一。
- 删除当前模板必须确认；删除后回到内置默认模板。删除模板不删除历史 Artifact。
- 工作区只记住上次选中的已保存模板；未命名草稿不跨重启恢复。

## 4. 字段模型

### 4.1 固有字段

首版提供三个稳定固有字段：

| 表达式 | 来源 | 输出规则 |
| --- | --- | --- |
| `{sheet.number}` | `Sheet.number` | 文本，保留前导零 |
| `{sheet.title}` | `Sheet.title` | 文本，原样输出 |
| `{sheet.file_name}` | `Sheet.layout.file_name` | 仅输出 Windows/Posix 路径共同语义下的 basename |

未附加格式码时，固有字段按上表原样输出；附加 §5.4 数字格式码后按格式语义输出。格式码可以跟在任意字段引用之后（包括 §4.2 的方括号形式），只作用于它紧跟的那一个引用。

固有字段名称为保留标识，始终优先于同名图纸自定义属性。

### 4.2 自定义属性

- `{sheetset.<属性名>}` 引用图纸集作用域自定义属性。
- `{sheet.<属性名>}` 引用图纸作用域自定义属性。
- 两种作用域可在同一表达式中组合。
- 名称匹配沿用 DST Manager 现有大小写不敏感规则；编译后绑定当前字段目录中的规范名称。
- 图纸集属性在每一输出行重复求值；图纸属性按当前图纸求值。

普通点号形式适用于可作为单一标识符读取且不与保留名称冲突的属性。包含空格、点号、引号、方括号、花括号，或与保留字段重名时使用 JSON 字符串形式的方括号语法：

```text
{sheet["title"]}
{sheet["专业.代码"]}
{sheetset["项目 名称"]}
```

字段浏览器必须根据属性规范名称自动插入正确形式，用户不需要自行判断何时转义。

### 4.3 定义与值

- “缺少属性定义”表示模板引用的字段不在当前图纸集对应作用域的字段目录中，是阻断错误。
- “缺少属性值”表示属性已经定义，但图纸集或部分图纸的值为空，是非阻断警告。
- 缺值求值为空字符串；表达式中的固定文字和分隔符仍原样保留。例如专业代码为空时 `{sheet.专业代码}-{sheet.number}` 输出 `-002`。
- 页面必须显示缺值涉及的字段及图纸数量；不默认列出全部属性值。

## 5. 表达式语言

### 5.1 语法

表达式是按顺序连接的字面量与字段引用：

```text
expression      := (literal | field_reference | escaped_brace)*
field_reference := "{" scope name_part format? "}"
name_part       := "." identifier | "[" json_string "]"
format          := ":" "0"{1,16}
scope           := "sheetset" | "sheet"
escaped_brace   := "{{" | "}}"
```

- `{{` 输出字面量 `{`，`}}` 输出字面量 `}`。
- 花括号外文字原样输出。
- 字段引用不能嵌套。
- `format` 是 §5.4 的数字格式码，宽度等于 `0` 的个数，取值 1～16；例如 `{sheet.number:0000}` 表示补零到 4 位。格式码只能出现在引用末尾，只作用于这一个字段引用，不作用于同一表达式中的其他引用或整列结果。
- 点号形式与方括号形式都允许附加格式码：`{sheet.number:0000}`、`{sheet["专业:代码"]:000}`。
- 空表达式、未闭合引用、未知作用域、非法转义和引用后多余内容均为语法错误。
- `{sheet.number:}`、`{sheet.number:abc}`、`{sheet.number:0:0}`、宽度超过 16 或引用未闭合均为语法错误，错误位置指向该引用的 `:`。
- 首版不支持函数、条件、默认值运算、循环、数学运算、过滤器、任意格式化管道或任意代码执行；表达式语言只为字段引用提供 §5.4 的单一数字格式码。
- 格式码引入前，`:` 在点号形式中属于禁用字符，在方括号形式后会留下未转义的 `}`；因此旧版本把带格式码的引用判为阻断语法错误，不会静默丢弃格式码，也不会导出与模板不符的数据。
- 实现必须解析为受限结构化 token；禁止 `eval`、`exec`、Jinja2 或其他通用模板执行器。

### 5.2 规范化与摘要

解析成功后，宿主把字段引用绑定为规范作用域和规范属性名，并生成确定性 token JSON。预览摘要至少绑定：

- `workspace_id`；
- `base_revision_id`；
- 模板 Schema 版本；
- 按顺序排列的输出列 ID、列名及规范化 token（token 含作用域、规范属性名和数字格式码宽度）；
- 扩展设置 Schema、revision 与规范化值 digest；`excluded_title_keywords` 通过该设置身份绑定，不在动作请求中复制；
- 扩展 ID、扩展版本和动作 ID。

任一项变化都会使旧 `preview_digest` 失效。未使用格式码时 token 形态与引入格式码前完全一致，既有模板的 `preview_digest` 不因本次扩展发生无谓变化。

### 5.3 限制

首版限制用于避免异常配置拖垮页面或生成器：

- 最多 100 个用户模板；
- 每个模板 1～50 列；
- 模板名 1～80 个字符；
- 输出列名 1～100 个字符，大小写不敏感地唯一；
- 单个表达式最多 1024 个字符；
- 数字格式码宽度 1～16 位（即 `:0` 至 16 个 `0`），超出按语法错误处理；
- 输出图纸过滤最多 50 个关键词，每项规范化后最多 100 个字符；
- 预览最多返回前 20 行，同时返回过滤后的输出行数和排除数量。

超限必须在保存和预览前明确提示，不进行截断或静默丢弃。

### 5.4 数字格式码语义

数字格式码是纯字符串变换，**不做数值转换**，只调整前导零，用于统一图号宽度；它不修改 DST、不写回任何属性值。设 `value` 为字段求值得到的原始字符串、`width` 为格式码中 `0` 的个数：

```text
format_value(value, width) =
    value                                       若 value 为空串，或不是纯 ASCII 数字串（^[0-9]+$）
    (value.lstrip("0") or "0").rjust(width, "0")  其余情况
```

| 原始值 | `{...:0000}` | `{...:0}` |
| --- | --- | --- |
| `1` | `0001` | `1` |
| `01` | `0001` | `1` |
| `0001` | `0001` | `1` |
| `0` | `0000` | `0` |
| `000` | `0000` | `0` |
| `00123` | `0123` | `123` |
| `12345` | `12345` | `12345` |
| 空值 | 空串 | 空串 |
| `A01`、`01A`、`1-2`、`1.2`、`１２３` | 原样 | 原样 |
| ` 01`（含空格） | 原样 | 原样 |

补充规则：

- **先归一化再补宽**：先去掉全部前导零、再左补零到 `width`，因此 `{...:0000}` 对 `00123` 输出 `0123` 而不是 `00123`。这样 `:0000` 的含义始终是“目标宽度 4 位”，与用户的 Excel 直觉（前导零不属于数据本身）一致，也对已规范的图号幂等。
- **去零保留一位**：全零值（`0`、`000`）使用 `:0` 得到 `0`，绝不产生空串。
- **不截断**：原值位数超过 `width` 时原样输出，不丢失字符。
- **不伪造空值**：空值空入空出，且仍按 §4.3 计入 `SHEET_CATALOG_VALUE_MISSING` 警告；格式码不会把空值补成 `0000`。
- **不 trim、不解析**：不处理空格、正负号、小数点、千分位、全角数字或任何非 ASCII 数字；含这些字符的值一律原样输出，不产生警告。
- **仍是文本**：格式码结果继续以字符串单元格写入 XLSX（§9），Excel 中不会变成数值。
- **只读**：格式码只影响预览与导出的输出值，不修改 DST、不新增字段、不改变字段定义或值。

## 6. 模板契约

### 6.1 数据结构

```json
{
  "template_id": "uuid",
  "name": "市政标准目录",
  "schema_version": 1,
  "columns": [
    {
      "column_id": "uuid",
      "header": "图纸编号",
      "expression": "{sheet.专业代码}-{sheet.number}"
    }
  ]
}
```

`template_id` 和 `column_id` 用于稳定识别、排序和未来升级，不能使用显示名称替代。

### 6.2 内置默认模板

内置默认模板固定为：

| 列名 | 表达式 |
| --- | --- |
| 图号 | `{sheet.number}` |
| 图名 | `{sheet.title}` |
| 文件名 | `{sheet.file_name}` |

默认模板随扩展版本交付，不写数据库、不可改名或删除。用户编辑后只能另存为新模板。

### 6.3 保存与兼容性

- 保存模板只要求 Schema、列名和表达式语法有效，不要求兼容当前图纸集。
- 选择或预览模板时再与当前字段目录进行兼容性校验。
- 不兼容模板仍保留并可编辑；列表和页面正文必须明显显示缺少的作用域与属性名。
- 扩展升级时按 `schema_version` 迁移；迁移失败保留旧 JSON 和诊断，不阻止宿主启动。

### 6.4 输出图纸过滤全局设置

图纸目录扩展设置 Schema 从 `1` 升为 `2`，与用户模板共用 ARCH-DM-006 定义的当前 Windows 用户级扩展设置记录：

```json
{
  "schema_version": 2,
  "user_templates": [],
  "excluded_title_keywords": ["草图", "作废", "TEMP"]
}
```

v1 → v2 迁移保留既有 `user_templates` 并把缺失的 `excluded_title_keywords` 解析为空数组；默认值不因迁移主动写回，仍遵循 Provider 的内存迁移与用户下次保存写回规则。持久化只保存规范化数组，不保存 UI 原始文本；规范化结果为空时移除该字段，GET 的有效值仍返回空数组，避免把默认值写成显式覆盖。

设置中心 custom 面板显示单行文本框“输出图纸过滤”，辅助说明为“图名包含任一关键词时不写入目录，多个关键词用逗号分隔”，占位示例为“草图, 作废, TEMP”。输入同时接受半角逗号 `,` 与全角逗号 `，`，Provider 按以下顺序规范化：

1. 以两种逗号拆分；
2. 去除每项首尾空白并忽略空项；
3. 按 Unicode `casefold()` 结果去重，保留第一次出现的原文和顺序；
4. 最多允许 50 项；超过 50 项或任一项超过 100 个字符时拒绝保存，并返回定位到 `excluded_title_keywords` 的字段级设置错误，绝不截断。

匹配对象是固有字段 `SheetSnapshot.title`（即界面和目录中的“图名”）。对图名与关键词分别执行 Unicode `casefold()` 后做字面子串匹配；任意关键词命中即排除，关键词之间为 OR。规则不支持通配符、正则、转义逗号或表达式求值。空数组等价于不过滤。

过滤发生在表达式求值和缺值统计之前，因此被排除图纸不产生 `SHEET_CATALOG_VALUE_MISSING`。全部图纸被过滤仍是可执行状态，生成只有表头的有效工作簿。

**计数语义与界面文案（PLAN-DM-025 R14，不得二义）**：预览响应的 `total_rows` 是**过滤后**的输出行数（即实际写入工作簿的图纸数），`filtered_rows` 是被排除的图纸数；全部过滤时 `total_rows=0`、`filtered_rows` 为原图纸数。业务页在既有预览区域显示「已过滤 N 张图纸」（N = `filtered_rows`），`filtered_rows=0` 时该提示不出现；它复用既有预览卡头，不新增区域、不新增列级控件、不列出被过滤图名。预览进行中不显示陈旧计数：提示与总行数取自同一份预览响应。

**与冻结 Demo 的关系（已声明，不重开 G4）**：该提示是 2026-09-12 §6.4 修订新增的界面正文，冻结截图 `default-light-1440x1000`/`default-dark-900x700` 早于本次修订，不含该提示；过滤证据见 §16 的 `production/g8-filter-*`（无冻结对照，属补充检查）。

## 7. 页面结构与状态

### 7.1 选定方向

页面采用用户于 2026-09-09 选定的“表达式模板”方向：顶部选择和管理模板；左侧字段浏览器；右侧“输出列名 + 表达式”编辑区；下方或同屏提供真实结果预览和主要导出操作。

已比较但未采用：

- 左右分栏片段构建器：易上手，但复杂组合表达受限，后续演进会引入第二套高级编辑方式。
- 分步向导：单步清晰，但多列模板需要频繁往返，难以整体检查和调整。

页面沿用 [SPEC-DM-006](SPEC-DM-006-dst-manager-desktop-ui-ux.md) 的外壳、令牌、焦点、主题和任务反馈模式，不新增独立视觉语言。

### 7.2 主要区域

1. 模板栏：当前模板、兼容状态、保存、另存为、删除。
2. 字段浏览器：分为“图纸固有字段、图纸集自定义属性、图纸自定义属性”；点击将正确语法插入当前表达式光标位置。每个字段条目另有格式入口，可直接插入 §5.4 格式码（去前导零，补零到 2/3/4/5/6 位），不需要用户手写语法。
3. 输出列编辑器：列名、表达式、校验状态、添加、删除和顺序调整。校验状态徽标取当前校验反馈：有反馈时按业务页语义显示「有效」或错误原因；**无任何校验反馈时显示中性「未校验」**，不伪造绿色「有效」（设置中心的扩展配置子面板不加载工作区快照、不伪造预览，2026-09-12 §6.4 修订引入）。**不提供列级格式控件**：格式码按字段生效，避免对 `{sheet.专业代码}-{sheet.number}` 这类组合结果整串补零（会得到 `0RQ-01`）。
4. 兼容性摘要：缺少定义为阻断错误；缺少值为带数量的警告。
5. 预览表：最多 20 行真实数据，显示过滤后的总行数；命中过滤词时另显示“已过滤 N 张图纸”，不列出被过滤图名；宽列只在受控内容区横向滚动。
6. 操作区：刷新预览、导出 XLSX；导出是唯一高强调操作。

### 7.3 状态矩阵

| 类别 | 必须覆盖的状态 |
| --- | --- |
| 页面 | 初始、加载、可用、空图纸集、加载失败、扩展停用、不兼容 |
| 模板 | 内置、用户模板、未命名草稿、已修改、保存中、保存失败、删除确认 |
| 表达式 | 有效、语法错误、未知字段、列名重复、超限 |
| 数据 | 全部有值、部分空值、图纸集字段缺失、图纸字段缺失、长字段名、长结果、大图纸集 |
| 输出图纸过滤 | 空设置、半/全角逗号、连续逗号、大小写混合、重复关键词、多个关键词 OR、部分过滤、全部过滤、数量/长度超限 |
| 预览 | 防抖等待、请求中、成功、过期、失败、修订漂移 |
| 导出 | 无桌面壳、选择路径、取消、生成中、成功、目标漂移、写入失败 |
| 视图 | 浅色、深色、基准视口、最小支持视口、200% 缩放、键盘操作 |

错误必须出现在可见正文中，并将焦点移到首个可操作问题；不能只用颜色、tooltip 或禁用按钮解释失败原因。

### 7.4 G4 可操作 Demo

[图纸目录交互 Demo](../mockups/SPEC-DM-012-sheet-catalog-demo.html)使用去敏虚构数据覆盖模板管理、字段插入、表达式编辑、兼容性提示、预览、空图纸集和模拟原生另存为。Demo 不调用真实 API、不创建 XLSX、不写模板数据库，也不登记 Artifact；具体差异和自动化预检证据见[图纸目录 G4 Demo 自动化 QA 记录](../../../.planning/memos/dst-manager/2026-09-09-sheet-catalog-demo-qa.md)。冻结版本为 commit `9f3dfb3`，基准截图为 [`1440 × 1000` 浅色](assets/SPEC-DM-012/default-light-1440x1000.jpg)和[`900 × 700` 深色](assets/SPEC-DM-012/default-dark-900x700.jpg)。

用户于 2026-09-09 确认认可 Demo，G4 已通过并冻结设计；冻结后如改变主流程、布局结构或关键状态，必须重新打开 G4。

## 8. 预览与执行接口

### 8.1 预览请求

```json
{
  "workspace_id": "uuid",
  "base_revision_id": "sha256",
  "template": {
    "template_id": "uuid-or-null",
    "name": "未保存草稿",
    "schema_version": 1,
    "columns": []
  }
}
```

预览响应包含：

- 规范化模板；
- 按作用域分组的当前字段目录；
- 阻断错误和非阻断警告；
- 前 20 行求值结果、过滤后的 `total_rows` 和排除数量 `filtered_rows`；
- `preview_digest`；
- `executable`。

只有语法、字段定义和限制全部通过时 `executable=true`。`filtered_rows=0` 时界面不显示过滤提示。空图纸集返回只有表头意义的有效预览，`total_rows=0`、`filtered_rows=0`，允许导出；全部图纸被过滤时同样允许导出，`total_rows=0`、`filtered_rows` 为原图纸数，且不返回被过滤图纸名称。

### 8.2 执行请求

执行请求重复提交模板快照，并携带：

```text
workspace_id
base_revision_id
preview_digest
save_grant_id
```

后端不得依赖前端缓存或仅凭 `template_id` 推断导出内容；必须重新解析模板、重新构建只读工作区与扩展设置快照并核对摘要。用户模板是否已经保存不影响执行。过滤关键词通过设置 Schema、revision 和 digest 绑定到预览；预览后修改过滤词时旧执行返回 `EXTENSION_SETTINGS_CHANGED`/409，且不得消费保存授权、生成候选文件或登记 Artifact。

执行成功响应包含 Artifact ID、文件名、用户选择的保存路径和非阻断警告。普通页面不显示来源修订、哈希和扩展版本；诊断接口仍可查询。

## 9. XLSX 输出规范

- 使用 `openpyxl` 生成标准 `.xlsx`。
- 工作簿只包含一个可见工作表，名称为“图纸目录”。
- 第一行为模板配置的列名；冻结首行并启用自动筛选。
- 数据从第二行开始，按 DST 子集顺序和图纸顺序输出，不自行排序。
- 写行前按 §6.4 的同一纯函数过滤图名；预览与 XLSX 不得各自实现一套匹配逻辑。
- 所有值以字符串单元格写入，保留 `001` 等前导零。
- 根据表头与预览/完整数据计算可读列宽并设置上限；超长内容换行。
- 不生成公式、宏、图表、外部链接、隐藏列、隐藏工作表或绝对工程路径。
- 空图纸集或全部图纸被过滤时生成只有表头的有效工作簿。
- 建议文件名为“`<图纸集名称>-图纸目录.xlsx`”；宿主替换 Windows 非法文件名字符并限制长度，用户可在原生对话框修改。

宿主生成后必须重新打开候选文件并核对工作表数、名称、表头、数据行数和禁止特性，再执行最终保存。

## 10. 保存位置与完成反馈

- 导出前必须通过桌面壳原生“另存为”选择最终位置，固定过滤器为 XLSX。
- 用户取消时不生成文件、不登记成功 Artifact，页面草稿和预览保持不变。
- 目标已存在时使用原生覆盖确认；确认后的目标身份仍纳入保存授权和执行前复核。
- 最终文件由宿主原子保存；扩展不得接收目标路径。
- 应用数据目录只保存临时候选，成功或失败后清理，不永久保留重复 XLSX。
- 成功提示正文显示最终路径，并提供“打开所在文件夹”；Artifact 技术信息默认隐藏。
- 文件之后被移动、修改或删除不影响工作区，只影响后台 Artifact 的可用性判断。

## 11. 错误规则

| 错误码 | 用户结果 |
| --- | --- |
| `SHEET_CATALOG_EXPRESSION_INVALID` | 定位到具体列和字符位置，禁止预览与导出 |
| `SHEET_CATALOG_FIELD_UNDEFINED` | 显示缺少的作用域和属性名，禁止导出 |
| `SHEET_CATALOG_VALUE_MISSING` | 显示字段和受影响行数，允许导出 |
| `SHEET_CATALOG_COLUMN_DUPLICATE` | 定位重复列名，禁止保存、预览和导出 |
| `SHEET_CATALOG_TEMPLATE_LIMIT` | 显示具体限制，不截断数据 |
| `SHEET_CATALOG_TEMPLATE_CONFLICT` | 保留本地编辑，刷新模板修订后允许另存或重试 |
| `EXTENSION_SETTINGS_INVALID` | 过滤关键词数量或长度超限时定位到 `excluded_title_keywords`，保留输入并拒绝保存 |
| `EXTENSION_SETTINGS_CHANGED` | 预览后过滤设置改变，保留编辑并要求重新预览 |
| `REPREVIEW_REQUIRED` | 保留编辑，刷新工作区字段与预览 |
| `SAVE_GRANT_INVALID` | 要求重新选择保存位置 |
| `EXPORT_DESTINATION_CHANGED` | 不覆盖目标，要求重新确认路径 |
| `SHEET_CATALOG_XLSX_INVALID` | 不保存候选文件，不登记成功 Artifact |
| `ARTIFACT_WRITE_FAILED` | 清理临时文件，保留编辑并允许重试 |

数字格式码错误（缺失、非法字符、重复、宽度超限）复用 `SHEET_CATALOG_EXPRESSION_INVALID`，错误位置指向该引用的 `:`。输出图纸过滤不新增图纸目录专用错误码，复用扩展设置框架的 `EXTENSION_SETTINGS_INVALID` 与 `EXTENSION_SETTINGS_CHANGED`。

错误响应使用稳定 `message_key` 与结构化参数。属性值、完整目标路径和表达式求值结果不得进入普通日志。

## 12. 安全与只读边界

- 扩展只使用 ARCH-DM-006 定义的冻结快照，不访问数据库、AcSm DOM、工作区对象或文件系统。
- 文件名由宿主裁剪为 basename；表达式无法访问绝对路径、Handle、布局路径或未声明字段。
- 表达式解析器不执行代码，不进行动态属性访问，不允许函数调用或网络访问。
- 数字格式码只是受限的前导零变换：不进行数值转换、不产生数值单元格、不引入新的输入面或参数系统。
- 输出过滤只执行规范化关键词的 `casefold()` 字面子串匹配，不执行正则、通配符、表达式或动态属性访问；日志和错误不得记录完整关键词或被过滤图名。
- XLSX 只包含值和受控样式，不写公式，防止用户属性被解释为公式；以 `=`、`+`、`-` 或 `@` 开头的值仍作为字符串单元格保存。
- 只有宿主处理保存授权、目标路径、覆盖基准和原子替换。
- 导出不取得工程写锁，不创建工程修订，不修改 DST/DWG 或工程内管理目录。

## 13. 兼容性与可访问性

- 扩展、模板和动作遵循 ARCH-DM-006 的版本契约；未知高版本模板不得加载为可编辑有效状态。
- 页面文案、字段分类、错误和原生对话框遵循 ARCH-DM-005 的中英文契约；DST 属性名和值保持原文。
- 所有模板、字段、列、预览和导出操作可由键盘完成。
- 表达式错误聚焦具体编辑框并提供字符位置；对话框定义 Esc、焦点圈闭和焦点归还。
- 状态同时使用文字和视觉表达；200% 缩放和最小视口下主要操作不被遮挡。
- 宽预览只允许内容区滚动，不造成整页横向溢出。

## 14. 技术映射

| 设计要求 | 当前实现 | 差距 | 数据/接口 | 修改位置 | 风险 | 验证方式 |
| --- | --- | --- | --- | --- | --- | --- |
| 扩展入口与启停 | `TabBar.vue` 固定三标签；`App.vue` 已 683 行 | 缺宿主贡献映射、注册表和生命周期 | `GET /api/extensions`、`PATCH .../state` | 新建 `extensions/contracts.py`、`registry.py`、`builtin/index.py`；拆出前端扩展页映射，不继续扩张 `App.vue` | 停用时活动调用与未保存输入 | 注册表单测 + E2E |
| 只读字段 | `WorkspaceResponse` 与领域 `Workspace` 已含 number/title/layout/custom properties/revision | HTTP 工作区响应包含的信息过宽，不能直接交给扩展 | `workspace.snapshot.read.v1` 冻结值对象 | 新建 `extensions/snapshots.py`、`capabilities.py` | Windows/Posix basename、路径泄漏、打开动作重复计算修订 | 快照表驱动测试 + DST/DWG 时间戳回归 |
| 属性定义 | `property_definitions_from_document` 已合并声明和实际图纸属性并按 `casefold()` 规范化 | 需按 `sheetset`/`sheet` 输出目录并保留规范名称 | `FieldCatalog` | `extensions/snapshots.py` 复用领域函数，不复制规则 | 保留字段与同名自定义属性冲突 | 领域/应用单测 |
| 模板与偏好 | SQLite 仅有通用 `application_settings`；设置中心文件存储不适合扩展业务 JSON | 缺隔离设置、乐观修订和工作区偏好 | settings/preferences GET/PUT | 新建 `extensions/settings.py`；迁移 `0006` 与独立仓储 | 旧 Schema、大小写重名、并发保存 | 迁移 + 仓储/接口测试 |
| 输出图纸过滤 |（**现状列为实施前快照**）当前扩展设置只存 `user_templates`，预览/执行遍历全部 `snapshot.sheets` | 缺 Provider Schema v2、custom 文本框、规范化过滤与过滤数量 | `excluded_title_keywords`、`filtered_rows`、设置 snapshot digest | PLAN-DM-025 的 `sheet_catalog/settings.py`、`preview.py`、`extension.py`、custom 设置面板与 OpenAPI 契约 | 预览/导出规则漂移、过滤后仍计缺值、设置变化执行旧预览 | 纯函数表驱动 + Provider 迁移 + API/Workbook + E2E/G8/G9 |
| 表达式与预览 | 无通用模板执行器 | 缺受限 parser、绑定、求值、摘要和兼容性诊断 | preview action | `builtin/sheet_catalog/expressions.py`、`extension.py` | 恶意输入、未知字段、缺值统计、摘要漂移 | 表驱动单测 + 集成测试 |
| XLSX | `pyproject.toml` 无 `openpyxl` | 缺生成与回读验证适配器 | 候选文件契约 | `builtin/sheet_catalog/workbook.py`；UV 增加锁定依赖 | 公式注入、前导零、超宽列、候选伪造 | `openpyxl` 回读测试 + 打包检查 |
| 保存位置 | `ShellBridge` 已支持打开/文件夹选择，API 与桥在桌面进程内装配 | 缺固定 XLSX SAVE_DIALOG、一次性授权和宿主原子落盘 | `request_save_xlsx()`、`SaveGrantStore.consume()` | 新建 `extensions/save_grants.py`、`artifacts.py`；修改 `interfaces/shell.py` 与桌面装配 | 授权复用、路径/用途错配、目标漂移、失败残留 | 桥/授权/故障注入单测 + G9 |
| Artifact | `database.py` 无成果模型且已 782 行 | 缺后台记录、查询和可用性派生；不应继续向大文件追加业务仓储 | `GET /api/artifacts/{id}` | 新建 `infrastructure/persistence/extensions.py`；迁移 `0006` 只放 ORM 表定义所需最小改动 | 文件后改动/删除、失败误登记 | 迁移 + 仓储/集成测试 |
| API 与错误 | `api.py` 已 512 行且错误仅有 `code/message` | 缺统一扩展分派端点和结构化错误；不能由扩展自挂 router | §8 请求/响应、`message_key/params` | 新建 `interfaces/extension_api.py`、`extension_contracts.py`；`create_app` 仅装配 router | OpenAPI 类型漂移、状态码不一致 | API 契约 + `npm run check:api` |
| 独立页面 | 已有共享主题、确认模态、toast、未保存对话框和标签键盘模型 | 缺目录页状态所有者、动态标签和保存桥客户端 | 编译期 `route_key -> Vue component` 映射 | 新建 `views/SheetCatalogView.vue`、`components/sheet-catalog/*`、`composables/useSheetCatalog.ts`；修改 `TabBar.vue`、`useShellTabs.ts`、`App.vue` 最小接线 | 50 列、200% 缩放、停用当前页、无桌面壳 | Playwright + G8 同状态截图 + G9 |

完整代码核对、状态所有权、文件容量裁决与 Spike 结论见[技术映射记录](../../../.planning/memos/dst-manager/2026-09-09-sheet-catalog-g5-technical-mapping.md)。当前没有阻断计划编制的未知高风险点；批次四须复用 ARCH-DM-005 的唯一 i18n 基础，不能另建局部翻译器。原生另存为、Excel 打开结果和覆盖/漂移行为仍按门禁保留为 G9 真实桌面验收，不以 mock 替代。

## 15. 测试与验收

### 15.1 后端自动化

- 清单缺失、重复、无效、不兼容、能力缺失和启动失败隔离；
- 启停持久化、活动调用排空、入口和动作完整移除；
- 快照字段裁剪、顺序、basename、两种属性作用域和修订身份；
- 表达式语法、花括号转义、方括号名称、保留名称冲突、大小写、未知字段和恶意输入；
- 数字格式码解析：点号与方括号两种形式、宽度 1 和 16 的边界、宽度 0/17、非法字符、重复冒号、未闭合引用，以及错误位置指向 `:`；
- 数字格式码语义：§5.4 表格逐行、先归一化再补宽、去零保留一位、不截断、空值空入空出且仍计缺值警告、非纯数字与全角值原样输出；
- 有格式码才改变 `preview_digest`，无格式码模板的摘要与本次扩展前逐字节一致；带格式码的值在 XLSX 中仍为字符串单元格；
- 缺定义阻断、缺值警告和分隔符保留；
- 模板 CRUD、内置模板不可变、大小写重名、上限、乐观并发和升级失败保留；
- 输出过滤 Provider：v1→v2 保留 `user_templates`、默认空数组、半/全角逗号、空项、首尾空白、Unicode 大小写不敏感去重、50 项与单项 100 字符边界；
- 输出过滤投影：多个关键词 OR、大小写不敏感字面包含、部分/全部过滤、排除行不计缺值警告、`total_rows`/`filtered_rows`、空表头 XLSX，以及预览/执行共用匹配函数；
- 预览摘要绑定工作区、修订、模板、扩展版本和动作；
- 预览摘要绑定设置 Schema/revision/digest；过滤设置变化返回 `EXTENSION_SETTINGS_CHANGED`/409，且授权未消费、无候选文件和 Artifact；
- 保存授权过期、复用、伪造、扩展/工作区错配、目标漂移和取消；
- XLSX 回读检查工作表、表头、顺序、字符串类型、前导零、筛选、冻结窗格和禁止特性；
- 原子保存各故障点不留半写文件，失败不登记成功 Artifact；
- 只读打开、预览和导出不创建 `.dst-manager/`，不改变 DST/DWG 时间戳；
- Alembic 全新升级及既有数据库升级回归。

### 15.2 前端与桌面验证

- Playwright 覆盖默认模板、用户模板、未保存确认、兼容、缺定义、缺值、空图纸集、预览漂移、保存取消和成功；
- Playwright 覆盖无工作区编辑输出过滤、规范化回显、字段级超限错误、部分/全部过滤汇总，以及过滤后预览与 XLSX 行数一致；
- 浅深主题、最小支持视口、200% 缩放、长属性名、50 列边界和大图纸集摘要；
- 键盘顺序、字段插入、错误聚焦、Esc 和焦点归还；
- 扩展停用后入口消失且核心三页面仍可用；
- 打包后 Windows 桌面壳真实验证另存为、覆盖确认、目标漂移提示、打开所在文件夹和 Excel 打开结果。

### 15.3 交付门禁

实施交付前至少运行：

```powershell
$env:UV_LINK_MODE = "copy"
uv sync --dev
uv run ruff check .
uv run pytest -q
uv lock --check
uv run alembic upgrade head

Set-Location web
npm ci
npm run build
npm run test:e2e
```

本功能不需要 AutoCAD；真实 AutoCAD 系统测试不是本功能的新增验收项，但既有非 CAD 与核心发布回归不得失败。

**门禁覆盖范围（已知缺口，必须知道它挡不住什么）**：`web/scripts/check-i18n.mjs` 只校验四类约束——中英域文件集合一致、域内键集合一致、命名插值参数一致、`web/src` 无未登记硬编码中文。它**不校验「代码引用的键是否存在」**：`t("extensions.sheetCatalog.settingsLoadFailed")` 这类引用即使键缺失，`check:i18n`、`check:api`、`vue-tsc -b` 与生产构建也都不会报错，只在运行期回退成键名原文（PLAN-DM-025 任务 8 评审 I2 的根因）。当前缓解手段：受影响的可见文案由 E2E 断言实际正文（如业务页设置加载失败提示、冲突横幅文案），并在评审中人工核对新增键的引用与登记；触发重开条件——若将来要求「构建期拦截未登记键引用」，需给 `check-i18n.mjs` 增加对 `t()` 字面量参数的反向扫描（本计划不修）。

**E2E 夹具与生产 digest 的一致性缺口（同上，不得当作已解决）**：`web/tests/e2e/fixtures/sheetCatalog.ts` 的 `preview_digest` 不参与列 UUID（生产 `preview.py` 的摘要载荷包含 `column_id`），且夹具不实现 `REPREVIEW_REQUIRED` 复核门禁。因此「换模板导致列 UUID 变化」与「预览后设置变化必须重新预览」两条语义在 E2E 里只被夹具的等价实现近似覆盖，**生产侧的权威约束**由后端实现与测试共同承担：`preview.py` 的 `_digest_columns` 把 `column_id` 计入摘要载荷，设置 Schema/revision/digest 绑定与 `EXTENSION_SETTINGS_CHANGED`/409 由 §15.1 的预览摘要与过滤设置变化两条回归覆盖。注意：现有后端摘要测试只固定 `column_id` 后比较 token/表头差异，**没有一条显式断言「仅列 UUID 变化即改变摘要」**。夹具保真度提升属后续项（PLAN-DM-025 任务 9 记录），不阻塞本 Spec。

## 16. 前端门禁记录

| 门禁 | 状态 | 证据 | 确认人 | 日期 | 未关闭事项 |
| --- | --- | --- | --- | --- | --- |
| G0 立项与范围 | 通过 | 本文 §1；ARCH-DM-006 §2 | 用户、技术负责人 | 2026-09-09 | — |
| G1 业务目标 | 通过 | 本文 §2 | 用户 | 2026-09-09 | — |
| G2 流程与状态 | 通过 | 本文 §3、§7.3、§10～§11 | 用户 | 2026-09-09 | — |
| G3 视觉方向 | 通过 | 本文 §7.1；用户选择表达式模板方向 | 用户 | 2026-09-09 | — |
| G4 Demo 与设计冻结 | 通过 | [Demo](../mockups/SPEC-DM-012-sheet-catalog-demo.html) commit `9f3dfb3` + [截图](assets/SPEC-DM-012/) + [自动化 QA 记录](../../../.planning/memos/dst-manager/2026-09-09-sheet-catalog-demo-qa.md) | 用户 | 2026-09-09 | 冻结后主流程、结构或关键状态变化须重开 G4 |
| G5 技术映射 | 通过 | 本文 §14 + [技术映射记录](../../../.planning/memos/dst-manager/2026-09-09-sheet-catalog-g5-technical-mapping.md) | 技术负责人（Agent） | 2026-09-09 | 原生另存为和 Excel 结果保留到 G9 真机验证 |
| G6 计划就绪 | 通过 | [PLAN-DM-020](../../../.planning/plans/dst-manager/PLAN-DM-020-sheet-catalog-builtin-extension.md)（含追踪矩阵、四批次和 12 个任务） | 技术负责人（Agent） | 2026-09-09 | 等待选择执行方式后进入 G7 |
| G7 分批实施 | 通过 | 2026-09-10 的实施与自动验证历史证据保留；2026-09-11 [MEMO-DM-031](../../../.planning/memos/dst-manager/2026-09-11-plan-dm022-final-review-defects.md) 确认 F1～F5 未被既有测试覆盖后重新打开，并由 [PLAN-DM-024](../../../.planning/plans/dst-manager/PLAN-DM-024-sheet-catalog-correctness-closure.md) 于同日收口（F1 `ecdc3d7`、F2/F3 `43eafa2`、F4/F5 `89cab15`，逐项关闭记录见 MEMO-DM-031 §7）。恢复时新鲜验证（全部退出码 0）：ruff 通过、聚焦 pytest 146 passed、`uv lock --check` 通过、`check:api`/`check:i18n`（870 键 / 9 域）/`build` 通过、聚焦 E2E（`--workers=1 --retries=0`）57 passed、全量 pytest 1121 passed / 72 skipped / 0 failed、全量 E2E 418 passed / 0 failed / 1 flaky（单独 `--workers=1 --retries=0` 复现通过，非真实回归） | 技术负责人（Agent） | 2026-09-11 | — |
| G8 设计 QA | 通过 | 2026-09-10 的 [原设计 QA 备忘（MEMO-DM-027）](../../../.planning/memos/dst-manager/PLAN-DM-020-sheet-catalog-design-qa.md)已被 2026-09-11 用户复验推翻，现行裁决见 [MEMO-DM-030](../../../.planning/memos/dst-manager/2026-09-11-plan-dm020-g8-user-revalidation.md)，整改由 [PLAN-DM-023](../../../.planning/plans/dst-manager/PLAN-DM-023-sheet-catalog-visual-convergence.md) 实施（commit `441b85c`/`1cf0a8f`/`0d69e45`/`70b23c0`/`241c7b5`/`6acc6e3`）；用户于 2026-09-11 逐对确认 V1～V8 全部关闭并接受 [MEMO-DM-030 §6.4](../../../.planning/memos/dst-manager/2026-09-11-plan-dm020-g8-user-revalidation.md) 的三项差异为可保留差异，记录见 MEMO-DM-030 §6.5 | 用户 | 2026-09-11 | 仅 A1（`↑`/`↓`/`✕` 图标列操作）、A2（生产壳层）与 §6.4 三项保留差异；G9 真实验收待用户执行 |
| G9 真实验收与关闭 | 未开始（待用户执行） | [G9 清单（MEMO-DM-028）](../../../.planning/memos/dst-manager/PLAN-DM-020-sheet-catalog-g9-checklist.md)的暂停已解除（G8 已于 2026-09-11 由用户重新确认通过），待用户执行：打包 Windows 壳 + 真实 Excel 逐项验收并填写操作者与日期 | 待用户 | — | 实施代理不代替填写 G9 结果；G9 通过前 PLAN-DM-020 保持 `active` |

G4～G8 已关闭；G7 曾因 2026-09-11 最终评审确认 F1～F5 重新打开，同日由 [PLAN-DM-024](../../../.planning/plans/dst-manager/PLAN-DM-024-sheet-catalog-correctness-closure.md) 关闭全部缺陷后恢复“通过”；G8 因同日用户真实桌面复验未通过而重新打开，G4 冻结设计继续有效，整改由 [PLAN-DM-023](../../../.planning/plans/dst-manager/PLAN-DM-023-sheet-catalog-visual-convergence.md) 实施完毕后由用户于同日逐对确认重新通过（确认人与日期见门禁表）。下一步按 [MEMO-DM-028](../../../.planning/memos/dst-manager/PLAN-DM-020-sheet-catalog-g9-checklist.md) 执行 G9 真实验收；G9 通过前，PLAN-DM-020 不得标记 `completed`。只有用户选择新的主流程、布局结构或关键状态时才重开 G3/G4，生产实现偏离本身不是重开冻结设计的理由。

2026-09-12 修订（§5.4 数字格式码）的门禁影响：主流程、页面布局结构与关键状态类别均未改变（§7.2 区域 2 只是新增条目内格式入口，区域 3 明确不新增列级控件），因此 **G3/G4 不重开**；字段条目格式入口属于新控件，已由 PLAN-DM-026 任务 4 补足**自动化**门禁证据：打开格式菜单后的 1440×1000 浅色与 900×700 深色同状态截图 [g8-format-menu-light-1440x1000.png](assets/SPEC-DM-012/production/g8-format-menu-light-1440x1000.png)、[g8-format-menu-dark-900x700.png](assets/SPEC-DM-012/production/g8-format-menu-dark-900x700.png)，断言覆盖菜单盒在视口内、无整页横向溢出、真实 Tab 环可在 140 步内到达格式入口（`toBeFocused()`）、Enter 展开与 Esc 关闭（`aria-expanded` 回到 `false` 且选项列表消失、焦点归还），以及 200% 缩放下入口仍落在字段栏盒内；≤980px 限高 235px 时被裁切的菜单尾部由字段列表内部滚动可达（`expectActionReachableAfterScroll`）。上述结论仅为自动化证据，**不构成用户对 G8 的重新确认**：G8 的确认人、日期与三项保留差异仍以本表 G8 行（用户，2026-09-11）为准。G9 真实验收清单（MEMO-DM-028）已追加 `### 1.9`“导出 XLSX 中补零图号为文本单元格（如 `0001`，Excel 显示不丢前导零）且未使用格式码的模板导出结果与升级前一致”，该项由用户执行，实施代理不代填。

2026-09-12 修订（§6.4 输出图纸过滤）的门禁影响：图纸目录业务页只增加“已过滤 N 张图纸”汇总，不改变既有页面主流程和布局结构，因此 SPEC-DM-012 的 G3/G4 不重开；但该配置进入 PLAN-DM-025 新增的“设置 → 扩展 → 图纸目录 → 配置”custom 面板，必须随 SPEC-DM-011 的扩展配置入口重新通过 G4 后才能实施生产 UI。G8 同时覆盖 custom 文本框与目录页过滤汇总的浅深主题、最小视口和 200% 缩放；G9 追加“无工作区保存过滤词、真实项目部分/全部过滤、空表头 XLSX、修改设置后旧预览失效”四项，均由用户执行，实施代理不代填。

2026-09-13 生产证据批次（PLAN-DM-025 任务 9，§6.4 输出图纸过滤的自动化归档）：

- 新增两张生产同状态截图（视口/主题与冻结基准对齐，图放在 `assets/SPEC-DM-012/production/`）：[g8-filter-partial-light-1440x1000.png](assets/SPEC-DM-012/production/g8-filter-partial-light-1440x1000.png)（1440×1000 浅色，5 张排除 1 张：`输出 4 张图纸` + `已过滤 1 张图纸`）与 [g8-filter-all-dark-900x700.png](assets/SPEC-DM-012/production/g8-filter-all-dark-900x700.png)（900×700 深色，全部排除：`输出 0 张图纸` + `已过滤 5 张图纸` + 空态正文 + 导出仍可用）。
- 生产方：`web/tests/e2e/sheet-catalog-visual-evidence.spec.ts` 的两例「G8 补充：1440×1000 浅色部分过滤截图 + 行数与几何守卫」「G8 补充：900×700 深色全部过滤截图 + 空态与几何守卫」，走**生产页面**（同名夹具装配，非冻结 Demo，`file://` 不可达该状态）；默认只挂测试附件，仅在 `DST_MANAGER_WRITE_G8_EVIDENCE=1` 时写入版本库目录。
- **这两张没有冻结对照**：冻结基准 `default-light-1440x1000.jpg`/`default-dark-900x700.jpg` 产生于 2026-09-09（commit `9f3dfb3`），其演示数据没有过滤设置，因此不存在「仅差过滤提示」的同状态冻结件。它们只作**补充检查**（几何守卫：无整页横向溢出、提示元素在视口内、导出可达），**不得写成比对通过或 G8 重新确认**。
- 与本批次同时归档的扩展配置入口/子视图证据（`assets/SPEC-DM-011/production/g8-ext-06～g8-ext-10`）覆盖了自定义设置面板：其中与冻结件 `g4-15` 的差异、模具与口径声明见 SPEC-DM-011 §7。图纸目录侧受影响的两处生产事实在此登记：① 设置中心的列编辑器**无校验反馈时显示中性「未校验」**，不伪造「有效」（§7.2 区域 3，业务页行为不变）；② custom 面板复用生产 `TemplateBar`/`ColumnEditor`/过滤行，与冻结 Demo 的可见差异（组标题文案、标签与控件同行/竖排、模板组承载完整模板栏与列表格）属已声明差异，**不重开 G3/G4**（主流程、布局结构与关键状态类别未变）。
- 冻结件（`g4-*`、`g8-catalog-*`、`g8-format-menu-*`、`default-*`）本批次**未重取、未覆盖**；G8 的确认人与日期仍以本表 G8 行（用户，2026-09-11）为准；G9 真实验收待用户在真实桌面与 Excel 环境执行。
