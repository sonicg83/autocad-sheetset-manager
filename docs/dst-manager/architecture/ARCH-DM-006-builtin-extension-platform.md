...
- 扩展停用、失败或不兼容时移除页面入口；
- **页面入口的移除不等于启停入口消失**：宿主级启停入口常驻在设置中心（[SPEC-DM-011](../specs/SPEC-DM-011-settings-center-ui.md) SC-15）。若启停控件只存在于扩展自己的页面上，停用会连同唯一可操作入口一起移除，开关即变成单向且不可逆（启停意图持久化在 `extension_states`，重启对账仍会重新停掉），用户被永久卡死。因此扩展页面不得自带停用控件，启停一律经设置中心并由宿主编排；
- **需要叠在已有模态之上的闸门/确认模态必须是原生模态**：设置窗口是原生 `<dialog showModal>`，其 top layer 会让窗口外内容 inert。宿主未提交输入三选一（`UnsavedInputDialog`）与目录页三选一因此均为 `<dialog showModal>`，由平台自行叠在设置窗口之上（选遮罩 `::backdrop`、焦点圈闭与 Esc 归属一并由平台保证，Esc 只作用于最上层模态）。若写成页面内联遮罩，会被上层模态 inert：用户看得见弹框却点不动，PATCH 永远不会发出。也因此，宿主级停用**不需要**先关闭设置窗口；
...
- PUT 必须携带 `expected_revision`，成功后修订递增，竞争写入返回 `409`。**修订冲突不新增错误码，复用 `EXTENSION_SETTINGS_INVALID`**：当前唯一真实触发点是 `src/dst_manager/application/extensions/settings.py` 的 `put()`——Store 抛出 `SettingsRevisionConflictError`，运行时捕获后映射为 `EXTENSION_SETTINGS_INVALID` + HTTP `409`，并在 `params` 中给出 `expected_revision` 与 `current_revision`。因此同一 PUT 端点出现三个必须区分的信号：请求体不合法（HTTP `422`，三种形态：Schema 版本不符 → `EXTENSION_SETTINGS_INVALID` 且 `params={settings_schema, submitted}`；请求体结构（Pydantic）校验失败 → `422` 但既无稳定 `code` 也无 `params`；字段级 Provider 校验 → `EXTENSION_SETTINGS_INVALID` 且 `params` 只带 `extension_id` 或字段定位）、已存 Schema 过新（`409` + `EXTENSION_SETTINGS_SCHEMA_NEWER`）、修订冲突（`409` + `EXTENSION_SETTINGS_INVALID` + 修订参数对）。**Provider 级校验失败不以「修订参数」表达**：它不新增错误码，统一用 `EXTENSION_SETTINGS_INVALID`（`params` 只带 `extension_id`/字段定位，HTTP `422`），或保留域自己的能力码——域码映射为 `409` 的（如 `SHEET_CATALOG_COLUMN_DUPLICATE`、`SHEET_CATALOG_TEMPLATE_CONFLICT`）同样**不带修订参数**；未被域状态映射表登记的域码回落到 `503`（`extensions/builtin/sheet_catalog/settings.py` 的 `_CATALOG_ERROR_STATUS.get(code, 503)`），因此「Provider 校验失败一律 `422`」并不成立。客户端因此**不得**只按错误码或参数判断冲突，完整判别式是「HTTP `409` **且**（码在冲突码集合内 **或** `params` 同时带 `expected_revision`/`current_revision`）」：`status` 前置在构造冲突对象时就已施加（`web/src/composables/useExtensionSettings.ts` 的 `save()` 只在 `status === 409` 分支构造冲突、非 409 归入普通失败态），`isRevisionConflict` 只在该冲突对象上判别码与参数；缺了 `409` 前置，Provider 的 `422` 字段错误就会进入冲突态并给出「按新修订重试」的无效出路。开发侧口径见 [GUIDE-DM-005](../guides/GUIDE-DM-005-builtin-extension-development.md) C-6。
