...
本文件不读取文件系统（随包 XSD 资源除外），不依赖 AutoCAD/DST codec，只做纯 DOM 契约判断。

所有权（PLAN-DB-001 Task 6）：自 ``dst_manager.infrastructure.acsm_xml.contract``
迁入；Manager 原路径仅保留薄 re-export，本模块是唯一实现。
"""
